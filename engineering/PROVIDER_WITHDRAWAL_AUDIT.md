...
| carrier command mount      | mount 属于 provider；每条 publication/installer cleanup 属于调用 caller-bound `bind()` 的 owner  | retained owned command 在 binding 撤回后返回 not found            | 同时持有 provider 与 publication owner admission；任一 stop 均 abort/drain     | replacement 建新 mount/binding；旧 route 不跟随同名或 schema-compatible implementation |
...
| Vault                      | root mount 共享；KV/blob handle 固定 Plugin/Part/caller owner                                    | 旧 handle 的异步读写与 flush 拒绝；同步 blob describe 无 IO       | owner + root lease 覆盖完整事务与 blob IO，stop 等待 settle，不强制中断事务    | Host root gate 先关闭/drain，服务最后 flush；新 generation 创建新 owner view           |
...
Vault 的服务作者实现从 `@pluxel/core/host` 使用 `enterOwnerInvocation()`，与 Core graph 使用同一 gate；没有第二份 owner registry。KV 在 snapshot read/mutation 集中边界接纳，blob 在外层完整 IO 接纳。最终宿主 flush 走内部管理路径，不尝试重新进入已经关闭的用户 admission。回归证据：`packages/services/tests/vault-lifetime.test.ts` 的 generation transaction drain、缓存 handle 拒绝，以及 root blob drain → 重开读取。
