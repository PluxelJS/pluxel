...
| commands                   | root catalog 共享；registration 绑定注册者 effects                                       | publication 消失后 installed handle 按 catalog 规则返回 not found | 已接纳 command 收到合成 abort signal，stop 等待 invocation release          | registry 属于 root；handler/registration 属于 generation                                |
...
| Workbench View             | `publish()` 绑定 owning Plugin effects；每次 `openView()` 建 fresh roots 和 owner leases | socket epoch/handle 关闭后 stubs 不能继续调用                     | owner stop 关闭 admission、abort open signal 并等待已接纳 Cap’n Web call    | registry/MF inventory 属于 root；publication、opened roots、Bridge 属于 generation/open |
...
Workbench 没有独立的 query/event registration 层；View API 是 Plugin 直接返回的 `RpcTarget`。Withdrawal 固定为：

1. Plugin generation 开始停止时关闭 owner invocation admission；
2. 撤销 definition publication 并推进 registry revision；
3. abort 对应 opened View signals；
