...
| Vault                      | root mount 共享；KV/docs/blob handle 固定 Plugin/Part/caller owner                               | 旧 handle 的异步读写与 flush 拒绝；同步 blob describe 无 IO       | owner + root lease 覆盖完整事务与 blob IO，stop 等待 settle，不强制中断事务    | Host root gate 先关闭/drain，服务最后 flush；新 generation 创建新 owner view           |
