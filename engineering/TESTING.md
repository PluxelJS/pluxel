...
- 使用宿主服务的 Plugin 使用 `@pluxel/preset/test`；public host 不暴露 root Context、transaction 或 backend。
...
`@pluxel/preset/test` 的 `createServiceTestHost()` 直接组合 `createHost()`；Core 作者符号从 `@pluxel/core/test` 导入，不通过服务入口转发。
...
Workbench 作者使用 `@pluxel/preset/test` 的 `createWorkbenchTestHost()`；它复用同一个服务 host 的事务和生命周期，独立持有 Workbench 会话并报告未释放 lease。
...
framework 白盒 fixture 从 `@pluxel/preset/internal/test` 导入，不能成为普通 Plugin 的业务依赖或在线运行状态的替代。
