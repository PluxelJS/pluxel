...
## 6. Instance、driver 与 privileged protocol
...
1. compile-only matrix 覆盖 callback、tuple、fork、negative surface、matcher augmentation 与 static binding inference；
2. Core/Runtime host prototype 覆盖并发、rollback、partial lifecycle failure、replacement/stale target 与 teardown；
3. Workbench/local RPC prototype证明 deep-copy、transfer/dup、withdrawal 与无泄漏；
4. Auth privileged Management、S3/Fonts Workbench、Package Manager failure/commands、fork/replacement、static application 与 dynamic smoke 完成代表迁移；
5. versioned agent eval 在只给 `.d.ts`、quick start 与 fixture 的条件下通过；
6. exact import/receiver-aware zero gate、package exports、用户文档与 Tegami changelog 同步。
