...
> 状态：候选设计，尚未采纳或实现。本文只定义三条入口应共享的语法与必须保留的边界；具体实现仍需真实调用点迁移与独立 public API review。
...
const response = await host.http.fetch(new Request('http://local.test/orders'))
...
```ts
interface StaticRuntimeTestHost extends AsyncDisposable {
	...
	require<T extends PluginConstructor>(target: T): InstanceType<T>
	isRunning(target: PluginTarget): boolean
	dispose(): Promise<void>
}
```
...
- physical `origin`：这个 host 明确是 in-process；真实 static Vite 或 artifact smoke 使用真实 launcher。

`startupReport` 保留 static 产品已有的 partial startup/report 语义。Plugin start failure 未必使整个 static application boot reject，因此不能套用
RuntimeTestHost strict `start(Plugin)` 的 throw contract。普通测试可以直接对 `startup.entries` 使用 `toContainEqual`、`objectContaining` 等
...
dynamic 外层模块也是 canonical Runtime entry，而不是一份可以内联传入的普通 config value。breaking refactor 时应评估统一为：
