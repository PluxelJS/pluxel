...
事实只有：consumer 选择 tab placement，Wretch provider 提供 caller-bound settings API 与 renderer。目标：
...
- Font contribution fixture provider + target Attachment：provider API 列候选，optional target API 写 consumer-specific selection。
...
| consumer-owned choice  | Font provider API + target selection API             |
...
2. 打开 local View 只增加一个 API root；Attachment 只增加 provider + optional target root；wire inventory 没有 session/resource 中间 target；
