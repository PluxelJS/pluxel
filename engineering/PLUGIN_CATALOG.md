...
在 canonical host module 中从 `@pluxel/runtime` 导入 `pluginNodeAddressOf`，并对已 lower 的 catalog constructor 取 address：
...
`nodes` 精确匹配结构化 Plugin node address；`packages` 的每项只能是精确 package name，或以唯一末尾 `*`
表示的 package-name prefix。除此以外不提供 glob、正则或 callback，保证 static/dynamic host 使用同一可序列化契约。

同一 node address 只能由一个宿主分类声明。package pattern 同时命中时使用最长的 literal prefix；相同长度仍有
...
type PluginCatalogPreferences = {
	version: 2
	assignments: { owner: PluginNodeAddressSnapshot; groupId: string | null }[]
	groupOrder: string[]
	pluginOrder: { groupId: string; owners: PluginNodeAddressSnapshot[] }[]
}
```
...
`groupOrder` 和 `pluginOrder` 只是已知 group/node 的稳定排序提示。读取时忽略未知、重复和失效项；暂时消失的 node
assignment 可以保留，以便相同结构化 node address 重新出现时恢复。写入 API 必须拒绝未知 group、伪造 package group、
未知 node、重复 membership 和非 canonical group name，不能把无效输入静默保存。
...
偏好文件只接受当前 version 2 结构化 owner schema；其他版本 fail-fast，不从名称或旧完整布局猜测转换。
...
- static node address 规则、dynamic exact package、最长 prefix 与冲突拒绝；
- disabled/stopped catalog entry 仍分类；
- 用户移动、明确未分组、恢复默认、排序和无效 mutation 拒绝；
- 新安装 package 自动出现、卸载消失、同 ID 重装恢复偏好；
- 非法/旧 preference version 与非法 node address 拒绝。
