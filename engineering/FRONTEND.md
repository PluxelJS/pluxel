...
Workbench frontend 是一个固定 Shell，既能直接渲染 Standard Page，也能按需加载完整 Plugin applications。业务 HTTP 与 Workbench 正交；
...
- `@pluxel/runtime/workbench`：browser-safe Page/View/Attachment definition；
- `@pluxel/runtime/workbench/client`：layout、portable Page plan 与 opened entry handle validation；
...
每个含 renderer 的 Plugin definition 的 UI producer 并验证这条反向依赖边界。Standard Page Markdown 在 build time 降为
portable plan，不进入 browser module graph。
...
MF manifest、JS/CSS、字体/图片等静态文件仍走 HTTP；Standard Page plan 随现有 `openView()` RPC 返回，不增加 artifact
...
discriminated pinned Page/MF reference，不包含 Page content 或任意 Plugin API dictionary。
...
current layout entry
  ...
  -> Standard Page: validate portable plan -> Shell render
  -> federated View: fresh root(s) -> pinned MF/Bridge activation
```

Standard Page open 只做短 owner admission；返回 immutable plan 后立即释放，不创建 host facade、imperative root、MF/Bridge 或
server-side Page lease。Federated View 只有整个 tuple 成功才成为 active：
...
Standard Page renderer 只消费 client boundary 二次验证后的封闭 AST，并以 React semantic elements 输出。Text 由 React escaping；
没有 raw HTML、runtime Markdown parser、图片或 Plugin component。Fragment DOM id 用每个 mounted Page 的 `useId()` scope，
同一文档打开多次也不会碰撞。
...
Page-only Plugin 没有 producer，不参与 shared compatibility 或 Bridge activation。

Development Page/UI update 先构建并验证完整 immutable candidate。Mixed Page/MF tuple 原子提交；失败保持当前完整 inventory，成功 commit 后完整 document
...
- layout runtime、Page/View activation 和 handle/Bridge cleanup 用 mount count + microtask cleanup 吸收 effect replay；
