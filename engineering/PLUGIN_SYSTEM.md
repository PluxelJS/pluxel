...
plugin source
	...
	└─ Workbench Definition / Markdown or renderer declarations
          ↓
...
插件只通过 `ctx.workbench?.publish(definition)` 或 `publish(definition, bindings)` 发布固定 Definition。宿主通过顶层 `workbench`
