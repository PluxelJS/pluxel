import type { FieldLayout } from '../../../core/meta'
import type { FieldNode, FieldKind } from '../../../core/fields'
 ...
 * 3. 可通过 layout.full 或 layout.span 手动控制
 */

/** 复杂类型集合（需要占满整行的字段类型） */
const COMPLEX_TYPES = new Set<FieldKind>(['object', 'array', 'union', 'record'])

/** 判断是否为复杂类型 */
export function isComplexType(type: FieldKind): boolean {
	return COMPLEX_TYPES.has(type)
}

export function isCompactField(field: FieldNode): boolean {
	...
	switch (field.kind) {
		...
		case 'picklist':
			return true
		case 'string': {
			const control = (field as any).control
			return control !== 'textarea' && control !== 'code'
		}
		default:
			return false
	}
...
/** 计算紧凑字段数量（排除复杂类型） */
export function countCompactFields(fields: FieldNode[]): number {
	return fields.filter((field) => isCompactField(field)).length
}
...
export function alignToCss(align?: FieldLayout['align']): string {
	switch (align) {
		case 'center':
			return 'center'
		case 'end':
