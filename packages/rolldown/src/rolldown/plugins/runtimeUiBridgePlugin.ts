/**
 * Rolldown/Vite plugin to lower `@pluxel/runtime/plugin` `ui(...)` declarations into
 ...
 * - Remove runtime bridge semantics from production build outputs.
...
export interface RuntimeUiBridgePluginOptions {
...
const RUNTIME_PLUGIN_SOURCE = '@pluxel/runtime/plugin'
const CODE_HINT = /@pluxel\/runtime\/plugin/

export function runtimeUiBridgePlugin(options: RuntimeUiBridgePluginOptions = {}): ViteCompatPlugin {
	...
	return {
		...
		transform: {
			...
			handler(this: TransformPluginContext, code, id) {
				...
				try {
					const rewritten = rewriteRuntimeUiBridge(code, ast)
					if (!rewritten) return null
					return { code: rewritten, map: null as null }
				} catch (error) {
					if (error instanceof Error && error.message.startsWith('[pluxel-runtime-ui-bridge]')) {
						this.error(error.message)
					}
					this.warn(`Failed to rewrite runtime ui bridge in ${id}: ${error}`)
					return null
				}
			},
		},
...
function rewriteRuntimeUiBridge(code: string, ast: Program): string | null {
	...
	for (const node of importDecls) {
		if (String(node.source.value ?? '') !== RUNTIME_PLUGIN_SOURCE) continue
		if (!Array.isArray(node.specifiers) || node.specifiers.length === 0) continue
		assertSupportedRuntimeImports(node)
...
function assertSupportedRuntimeImports(node: ImportDeclaration): void {
	for (const spec of node.specifiers ?? []) {
		if (spec.type === 'ImportDefaultSpecifier') {
			throw new Error(
				`[pluxel-runtime-ui-bridge] default import is not supported for ${RUNTIME_PLUGIN_SOURCE}; use named imports like \`import { ui, worker } from '${RUNTIME_PLUGIN_SOURCE}'\``,
			)
		}
		if (spec.type === 'ImportNamespaceSpecifier') {
			throw new Error(
				`[pluxel-runtime-ui-bridge] namespace import is not supported for ${RUNTIME_PLUGIN_SOURCE}; use named imports like \`import { ui, worker } from '${RUNTIME_PLUGIN_SOURCE}'\``,
			)
		}
	}
}
...
function buildHelperBlock(helperName: string, localNames: string[]): string {
	...
	return [
		'',
		'// [pluxel-runtime-ui-bridge] Injected runtime packaged bridge',
		`function ${helperName}(input) {`,
		"\tconst entryPath = typeof input === 'string' ? String(input).trim() : String(input?.entryPath ?? '').trim()",
		"\tif (!entryPath) throw new Error('[pluxel/runtime] ui(): entryPath required')",
		'\treturn {',
		'\t\tbind(ctx) {',
