 ...
 */
export function runManagementFederationBuild(task: Task): Promise<void> {
	return federationBuildQueue.run(task)
}

/** Serializes build, validation, and atomic publication for one artifact target. */
export function runManagementOutputTransaction(target: string, task: Task): Promise<void> {
