...
describe('buildManagementUiRemote', () => {
	it('rejects partial cached artifacts including missing lazy chunks', async () => {
		...
		await expect(
			validateManagementUiArtifact(join(fixture.path, 'artifact'), 'PluginWithUI'),
...
`,
		...
		const first = await buildManagementUiRemote({
			root,
		...
		const second = await buildManagementUiRemote({
			root,
			pluginName: 'PluginWithUI',
			entryPath,
...
`,
		...
		const root = join(fixture.path, 'packages/plugins/demo')
		const outDir = join(fixture.path, 'dist/management-ui')
		await buildManagementUiRemote({
			...
			vite: {
				plugins: [
					{
						name: 'test:management-ui-transform',
						transform: {
							filter: {
								id: /\/src\/ui\/index\.ts$/,
							},
...
`,
	...
	it('creates a stable signature for user build additions', () => {
		const signature = resolveManagementUiBuildSignature({
