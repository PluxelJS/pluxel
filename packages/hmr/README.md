...
未来目标态会删除这个包，把能力收敛为 `@pluxel/runtime-dynamic` 的 HMR mode，并且不保留 `@pluxel/hmr` 或 `@pluxel/hmr/*` 兼容入口。目标设计见 [`docs/proposals/runtime-dynamic-hmr-mode.md`](../../docs/proposals/runtime-dynamic-hmr-mode.md)。本文只描述当前已实现入口。
...
以下公开面属于当前实现，不是目标态兼容承诺。未来 runtime-dynamic HMR mode 落地时会一次性删除这些入口。
