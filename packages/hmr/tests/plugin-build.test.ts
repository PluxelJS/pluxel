...
describe('buildPluginUiRemote', () => {
	...
	it(
		'keeps legacy federation temp directories untouched while reusing child-process isolation',
		async () => {
			await using fixture = await createFixture({
				'packages/plugins/demo/package.json': JSON.stringify({
					name: '@pluxel/plugins-demo',
			...
			await expect(readdir(join(root, '.pluxel/vite-plugin-ui-cache')).catch((): string[] => [])).resolves.toEqual([])
		},
		20_000,
	)
})
