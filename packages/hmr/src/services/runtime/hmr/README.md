...
- Warmup reuses the same entry list as the startup report (`startupScope`).
- Prefer passing `hmrService.entries` from a workspace-profile snapshot (recommended).
...
- `environment.ts`: path normalization, id variants, scan-root filtering.
