import { HMR_LOG_STREAMS_BASE } from '@pluxel/hmr-web'
...
export const logRoutes = (app: AnyElysiaApp) =>
	app.group(HMR_LOG_STREAMS_BASE, (streams) =>
		streams
			...
			.get('/:streamId/range', (c) =>
				withResolvedStream(c.params.streamId, () => c.status(404, 'Stream not found'), (resolved) => {
			...
			.get('/:streamId/follow', (c) => {
				...
				const resolvedFrom = resolveFromSeq(url.searchParams, c.request.headers.get('Last-Event-ID'))
					...
					async (sse) => {
						...
						const enqueueAppendLines = (
							...
							if (hasFilter && opts.alreadyFiltered !== true) {
								filtered = sliced.filter((line) =>
									matchesLogFilterCompiled(line, compiledFilter),
								)
							}
