...
export function formatHmrUpdateSummary(input: {
	...
	plugins?: { loaded: number; enabled: number; running: number }
	invalidated: { vite: number; runner: number }
	commitMs: number | null
	batchMs: number
	hotspots?: ReadonlyArray<{ id: string; ms: number }>
	commitOk: boolean
	commitError?: string
}): string {
	const changed =
		input.changedPreview.length > 0
			? input.changedPreview.join(', ')
			: input.changedPreviewOmitted
				? '(omitted)'
				: '(none)'
	const changedOmitted =
		input.changedPreviewOmitted && input.changedPreviewOmitted > 0
			? ` (+${input.changedPreviewOmitted} more)`
			: ''

	const hotspot =
		input.hotspots?.length && input.hotspots[0]
			? (() => {
					const top = input.hotspots!.slice(0, 3)
					const list = top
						.map((h) => {
							return `${h.ms.toFixed(1)}ms ${h.id}`
						})
						.join('; ')
					return `hotspotsTop(3)=[${list}]`
				})()
			: null

	const status = input.commitOk ? 'success' : 'failed'
	const commit = input.commitMs === null ? '-' : input.commitMs.toFixed(1)
	const batch = input.batchMs.toFixed(1)
	const plugins = input.plugins
		? `${input.plugins.loaded}/${input.plugins.enabled}/${input.plugins.running}`
		: null

	const header =
		`[HMR] updated epoch=${input.epoch} status=${status} ` +
		`batchMs=${batch}ms commitMs=${commit}ms`
	const stats1 =
		`  changedFiles=${input.changedFiles} ` +
		`targets=${input.targets} ` +
		`affected=${input.affected} ` +
		`fallbackRoots=${input.fallbackRoots} ` +
		`activeServices=${input.activeServices}`
	const stats2 =
		`  invalidated.vite=${input.invalidated.vite} ` +
		`invalidated.runner=${input.invalidated.runner}` +
		(plugins
			? ` plugins.loaded=${input.plugins!.loaded} plugins.enabled=${input.plugins!.enabled} plugins.running=${input.plugins!.running}`
			: '') +
		(hotspot ? ` ${hotspot}` : '')

	const errorLine =
		input.commitOk || !input.commitError
			? null
			: `  commitError=${String(input.commitError).replace(/\s+/g, ' ').slice(0, 240)}`
	...
	return [
		...
		stats1,
		stats2,
		`  changedPreview(${input.changedPreview.length})=[${changed}]${changedOmitted}`,
	]
		.filter(Boolean)
		.join('\n')
}
