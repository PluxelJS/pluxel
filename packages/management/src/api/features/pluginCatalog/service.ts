...
import { CatalogLayout } from '../../../token'
import { pluginDefinitionIndexKey, type Context } from '@pluxel/core'
import { type HostOperationOptions, requirePluginHostCoordinator } from '@pluxel/host/internal'
import type {
...
export async function readPluginCatalog(
	...
	const pinned = await readPinnedCatalog(ctx, options)
	const sections = await layout.listSections(pinned.layoutEntries, options)
...
export async function writePluginCatalogLayout(
	...
	const layout = requirePluginCatalogLayout(ctx)
	const pinned = await readPinnedCatalog(ctx, options)
	const sections = await layout.updateSections(input.sections, pinned.layoutEntries, options)
	return sections.map(toSection)
}

function requirePluginCatalogLayout(ctx: Context) {
	const service = resolveContextCapability(ctx.root, CatalogLayout)
	if (!service) {
...
async function readPinnedCatalog(ctx: Context, options?: HostOperationOptions) {
