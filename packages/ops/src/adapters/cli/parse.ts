	...
	if (tailSpec.mode === 'parsebox') {
		if (!parseboxTail) {
			throw new OpError('E_INTERNAL', 'Internal error', {
				message: 'ParseBox tail parser is missing',
			})
		}
		const parseboxInput = rawText.endsWith('\n') ? rawText : `${rawText}\n`
		const parsed = parseboxTail.module.Parse(parseboxTail.entry, parseboxInput) as unknown
...
const joinRawTokens = (tokens: CliToken[], start: number) => {
	let out = ''
	for (let index = start; index < tokens.length; index += 1) {
		if (index > start) out += ' '
		out += tokens[index]!.raw
	}
	return out
}

export const buildParamAliases = (params: ParamSpec[]) => {
...
const parseObjectCandidate = (
	...
	for (let index = start; index < tokens.length; index += 1) {
		...
		if (token.value.startsWith('--')) {
			...
			if (spec.type === 'boolean' && separator < 0) {
				...
				if (
					!next ||
					looksLikeKeyedParam(next, entry.paramAliases) ||
					!booleanLiterals.has(next.value.toLowerCase())
				) {
	...
	if (entry.tail?.mode === 'parsebox' && tailStart >= 0) {
		const patch = parseTail(entry.tail, joinRawTokens(tokens, tailStart), entry.parseboxTail)
		const properties =
		...
		const firstTailToken = tokens[tailStart]
		if (
			!explicitTail &&
			firstTailToken &&
			looksLikeKeyedParam(firstTailToken, entry.paramAliases)
		) {
			throw new OpError('E_CLI_PARSE', 'Invalid command input', {
		...
		candidate[tailKey] = parseTail(entry.tail, joinRawTokens(tokens, tailStart))
	}

	return candidate
}

const parsePrimitiveCandidate = (
	schema: Record<string, unknown>,
	tokens: CliToken[],
	start: number,
) => {
	if (tokens.length <= start) return undefined
	const raw = joinRawTokens(tokens, start)
	switch (schema.type) {
		case 'string':
			return raw
		case 'number': {
		...
		case 'integer': {
			...
			if (!Number.isInteger(value)) {
				throw new OpError('E_CLI_PARSE', 'Invalid command input', {
					message: 'Expected integer input',
				})
			}
			return value
		}
...
export const parseCandidate = (entry: CliParseEntry, tokens: CliToken[], start = 0) =>
	entry.inputSchemaJson.type === 'object'
		? parseObjectCandidate(entry, tokens, start)
		: parsePrimitiveCandidate(entry.inputSchemaJson, tokens, start)
