/**
 * Rolldown/Vite plugin to lower `@pluxel/runtime-loader/plugin` `ui(...)` declarations into
 * runtime packaged registration helpers during build.
 *
 * Goal:
 * - Keep author-facing source on `ui(...).bind(ctx)` for loader dev / AST recognition.
 * - Remove runtime-loader bridge semantics from production build outputs.
 * - Preserve the same local binding name so existing source structure keeps working.
 *
 * Example:
 *   import { ui } from '@pluxel/runtime-loader/plugin'
...
export interface RuntimeLoaderUiBridgePluginOptions {
...
const RUNTIME_LOADER_PLUGIN_SOURCE = '@pluxel/runtime-loader/plugin'
const CODE_HINT = /@pluxel\/runtime-loader\/plugin/

export function runtimeLoaderUiBridgePlugin(options: RuntimeLoaderUiBridgePluginOptions = {}): ViteCompatPlugin {
	...
	return {
		name: 'pluxel-runtime-loader-ui-bridge',
		enforce: 'pre',
		transform: {
			filter: {
				id: {
			...
			handler(this: TransformPluginContext, code, id) {
				...
				try {
					const rewritten = rewriteRuntimeLoaderUiBridge(code, ast)
					if (!rewritten) return null
					return { code: rewritten, map: null as null }
				} catch (error) {
					if (error instanceof Error && error.message.startsWith('[pluxel-runtime-loader-ui-bridge]')) {
						this.error(error.message)
					}
					this.warn(`Failed to rewrite runtime-loader ui bridge in ${id}: ${error}`)
					return null
				}
			},
		},
...
function rewriteRuntimeLoaderUiBridge(code: string, ast: Program): string | null {
	...
	for (const node of importDecls) {
		if (String(node.source.value ?? '') !== RUNTIME_LOADER_PLUGIN_SOURCE) continue
		if (!Array.isArray(node.specifiers) || node.specifiers.length === 0) continue
		assertSupportedRuntimeLoaderImports(node)
...
function assertSupportedRuntimeLoaderImports(node: ImportDeclaration): void {
	for (const spec of node.specifiers ?? []) {
		if (spec.type === 'ImportDefaultSpecifier') {
			throw new Error(
				`[pluxel-runtime-loader-ui-bridge] default import is not supported for ${RUNTIME_LOADER_PLUGIN_SOURCE}; use named imports like \`import { ui, worker } from '${RUNTIME_LOADER_PLUGIN_SOURCE}'\``,
			)
		}
		if (spec.type === 'ImportNamespaceSpecifier') {
			throw new Error(
				`[pluxel-runtime-loader-ui-bridge] namespace import is not supported for ${RUNTIME_LOADER_PLUGIN_SOURCE}; use named imports like \`import { ui, worker } from '${RUNTIME_LOADER_PLUGIN_SOURCE}'\``,
			)
		}
	}
}
...
function buildHelperBlock(helperName: string, localNames: string[]): string {
	...
	return [
		'',
		'// [pluxel-runtime-loader-ui-bridge] Injected runtime packaged bridge',
		`function ${helperName}(input) {`,
		"\tconst entryPath = typeof input === 'string' ? String(input).trim() : String(input?.entryPath ?? '').trim()",
		"\tif (!entryPath) throw new Error('[pluxel/runtime-loader] ui(): entryPath required')",
		'\treturn {',
		'\t\tbind(ctx) {',
