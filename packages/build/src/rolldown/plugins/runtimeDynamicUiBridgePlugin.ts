/**
 * Rolldown/Vite plugin to lower `@pluxel/runtime-dynamic/plugin` `ui(...)` declarations into
 * runtime packaged registration helpers during build.
 *
 * Goal:
 * - Keep author-facing source on `ui(...).bind(ctx)` for loader HMR / AST recognition.
 * - Remove runtime-dynamic bridge semantics from production build outputs.
...
export interface RuntimeDynamicUiBridgePluginOptions {
...
const RUNTIME_DYNAMIC_PLUGIN_SOURCE = '@pluxel/runtime-dynamic/plugin'
const CODE_HINT = /@pluxel\/runtime-dynamic\/plugin/

export function runtimeDynamicUiBridgePlugin(options: RuntimeDynamicUiBridgePluginOptions = {}): ViteCompatPlugin {
	...
	return {
		name: 'pluxel-runtime-dynamic-ui-bridge',
		enforce: 'pre',
		transform: {
			filter: {
				id: {
			...
			handler(this: TransformPluginContext, code, id) {
				...
				try {
					const rewritten = rewriteRuntimeDynamicUiBridge(code, ast)
					if (!rewritten) return null
					return { code: rewritten, map: null as null }
				} catch (error) {
					if (error instanceof Error && error.message.startsWith('[pluxel-runtime-dynamic-ui-bridge]')) {
						this.error(error.message)
					}
					this.warn(`Failed to rewrite runtime-dynamic ui bridge in ${id}: ${error}`)
					return null
				}
			},
		},
...
function rewriteRuntimeDynamicUiBridge(code: string, ast: Program): string | null {
	...
	for (const node of importDecls) {
		if (String(node.source.value ?? '') !== RUNTIME_DYNAMIC_PLUGIN_SOURCE) continue
		if (!Array.isArray(node.specifiers) || node.specifiers.length === 0) continue
		assertSupportedRuntimeDynamicImports(node)
...
function assertSupportedRuntimeDynamicImports(node: ImportDeclaration): void {
	for (const spec of node.specifiers ?? []) {
		if (spec.type === 'ImportDefaultSpecifier') {
			throw new Error(
				`[pluxel-runtime-dynamic-ui-bridge] default import is not supported for ${RUNTIME_DYNAMIC_PLUGIN_SOURCE}; use named imports like \`import { ui, worker } from '${RUNTIME_DYNAMIC_PLUGIN_SOURCE}'\``,
			)
		}
		if (spec.type === 'ImportNamespaceSpecifier') {
			throw new Error(
				`[pluxel-runtime-dynamic-ui-bridge] namespace import is not supported for ${RUNTIME_DYNAMIC_PLUGIN_SOURCE}; use named imports like \`import { ui, worker } from '${RUNTIME_DYNAMIC_PLUGIN_SOURCE}'\``,
			)
		}
	}
}
...
function buildHelperBlock(helperName: string, localNames: string[]): string {
	...
	return [
		'',
		'// [pluxel-runtime-dynamic-ui-bridge] Injected runtime packaged bridge',
		`function ${helperName}(input) {`,
		"\tconst entryPath = typeof input === 'string' ? String(input).trim() : String(input?.entryPath ?? '').trim()",
		"\tif (!entryPath) throw new Error('[pluxel/runtime-dynamic] ui(): entryPath required')",
		'\treturn {',
		'\t\tbind(ctx) {',
