...
This document records the `@pluxel/core` DI migration against the pre-migration `diod` implementation with benchmark facts only.
...
These archived measurements compare the specified migration commits and runner only. They show improvements in the listed lifecycle workloads, with unchanged rounded mean latency for the empty-commit case. They do not establish performance of later revisions or identify a bottleneck without profiling.
