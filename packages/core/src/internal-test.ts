...
export type {
	AssertPluginTestTargetCurrentOptions,
	PluginTestDraftAuthority,
...
export interface CoreInternalTestHost extends AsyncDisposable {
	readonly ctx: RootContext
	/** Explicit framework authority aliases. Prefer these over resolving services repeatedly. */
	readonly root: RootContext
	readonly pluginService: PluginService
	readonly configService: ReturnType<typeof requireConfigService>
	add(Plugin: PluginConstructor): CoreInternalTestHost
	add(Plugins: readonly PluginConstructor[]): CoreInternalTestHost
	remove(target: AnyTarget, options?: { cascadeDependents?: boolean }): CoreInternalTestHost
	remove(targets: readonly AnyTarget[]): CoreInternalTestHost
	restart(target: AnyTarget, options?: { cascadeDependents?: boolean }): CoreInternalTestHost
	replace(
		target: AnyTarget,
		next: PluginConstructor,
		options?: { cascadeDependents?: boolean },
	): CoreInternalTestHost
...
export type CoreInternalTestContext = {
	readonly ctx: RootContext
	readonly root: RootContext
	dispose: () => Promise<void>
}
export type CoreInternalTestHostOptions = {
...
export function createCoreInternalTestHost(
	config: CoreHostConfig = {},
	options: CoreInternalTestHostOptions = {},
): CoreInternalTestHost {
	let host!: CoreInternalTestHost
	...
	function add(Plugin: PluginConstructor): CoreInternalTestHost
	function add(Plugins: readonly PluginConstructor[]): CoreInternalTestHost
	function add(value: PluginConstructor | readonly PluginConstructor[]): CoreInternalTestHost {
	...
	function remove(targets: readonly AnyTarget[]): CoreInternalTestHost
	function remove(
	...
	host = {
		...
		get: get as CoreInternalTestHost['get'],
		require: requirePlugin as CoreInternalTestHost['require'],
		cfg: (<T extends PluginConstructor>(target: TypedTarget<T>): CoreHostConfigHandle<T> => {
		...
		[Symbol.asyncDispose]: async () => host.dispose(),
	}
	return host
}
...
export async function withCoreInternalTestHost<T>(
	fn: (host: CoreInternalTestHost) => Promise<T> | T,
	config: CoreHostConfig = {},
	options: CoreInternalTestHostOptions = {},
): Promise<T> {
...
export function createCoreInternalTestContext(
	config: CoreHostConfig = {},
	options: Pick<CoreInternalTestHostOptions, 'createRootContext'> = {},
): CoreInternalTestContext {
	const resolvedConfig = { name: 'test', ...normalizeConfig(config) }
	const ctx = options.createRootContext?.(resolvedConfig) ?? createCoreRootContext(resolvedConfig)
	return {
		ctx,
...
export async function withCoreInternalTestContext<T>(
	fn: (ctx: RootContext) => Promise<T> | T,
	config: CoreHostConfig = {},
): Promise<T> {
	const value = createCoreInternalTestContext(config)
	try {
