...
import { dynamicSource } from '../../host-dynamic/src/index'
import { startRuntimeApplication } from '../src/application/internal/application'
...
it('starts the initial dynamic catalog and accepts fresh built entries and withdrawal', async () => {
	...
	let runtime: Awaited<ReturnType<typeof startRuntimeApplication>> | undefined
	try {
		await writeFile(join(root, 'initial.mjs'), builtModule('initial'))
		runtime = await startRuntimeApplication(
			{
				...
				configure: () => ({
					...
					runtimeState: {
						mode: 'memory',
						snapshot: { autoStart: [address('initial'), address('added')] },
					},
				}),
			},
			{ startup: { root, mode: 'production', env: {}, bindings: {} } },
		)
		expect(runtime.startupReport.entries).toContainEqual(
			expect.objectContaining({ address: address('initial'), status: 'started' }),
		)
		...
		startRuntimeApplication(
			{
				...
				prepare({ host }) {
					...
					throw new Error('startup preparation failed')
				},
			},
