...
// Ensure @pluxel/runtime services (http/AuthGuardService/ExtService) are registered.
...
describe('AuthGuard end-to-end (http)', () => {
	...
	it('guards the internal API and HTML navigation; /auth verify unblocks; unload removes guard', async () => {
		host = createHost()
		...
		{
			const res = await host.ctx.http.fetch(req(internalUrl()))
		...
		const cookie = await (async () => {
			const res = await host!.ctx.http.fetch(req('http://local/auth/verify', { method: 'POST' }))
			expect(res.status).toBe(200)
