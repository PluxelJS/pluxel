...
Scope/resource 是 module-scoped declaration；每次 Bridge mount 创建独立 renderer owner，并随 opened handle 一起清理。
Snapshot/watch、mutation、portable detach 与低层 escape hatch 的完整契约只在
[Workbench 作者文档](../../docs/workbench/index.md)维护；内部所有权、admission 与 teardown 不变量见
[Workbench 架构](../../engineering/WORKBENCH.md)。Shell 的 Management client 位于 `/web`，Plugin renderer resource 位于
`/workbench/react`；两者共用 transport，但不共享 facade、cache 或生命周期。
