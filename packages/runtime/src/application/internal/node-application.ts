...
export type StaticNodeApplication = Runtime & {
	readonly address: { host: string; port: number }
}

export async function runStaticNodeApplication<TBindings extends RuntimeBindings = RuntimeBindings>(
	...
	const applicationCarrier = new NodeElysiaApplicationCarrier({
		...
		metadata: () => {
			if (!carrier.url) {
				throw new Error('[runtime] srvx Node carrier metadata is unavailable before ready')
			}
	...
	if (!nodeServer) {
		...
		await applicationCarrier.close().catch((): undefined => undefined)
		await runtime.stop().catch((): undefined => undefined)
		throw new Error('[runtime] srvx Node carrier did not expose an upgrade listener')
	}
	...
	const stop = () => {
		stopPromise ??= (async () => {
			...
			if (errors.length > 1) {
				throw new AggregateError(errors, '[runtime] Node carrier shutdown failed')
			}
		})()
		return stopPromise
	}
