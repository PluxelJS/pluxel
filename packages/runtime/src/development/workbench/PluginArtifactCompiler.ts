...
import { pluginDefinitionIndexKey, type Context, type NodeModuleDeclaration } from '../../index.ts'
import {
...
export class PluginArtifactCompiler {
	...
	async watchNodeModule(
		...
		try {
			...
			if (!url) throw new Error(`[host-dev] Node module build produced no artifact: ${key}`)
			let active = true
			return {
				url,
				dispose: async () => {
