...
import { getHmrRuntimeHandles } from '../../runtime/hmr-handles'
...
export function registerRuntimeHmrTools(server: McpServer, ctx: Context) {
	const requireHmr = () => {
		const handles = getHmrRuntimeHandles(ctx)
		const hmr = handles?.hmr
		if (hmr) return hmr

		throw new Error('HMR runtime not available (did you start the loader HMR runtime?)')
	}
