...
import type { WorkbenchEventsService, SseChannel } from './WorkbenchEventsService'

export interface SignalDbCollectionOptions<T extends SignalDbItem> {
	name: string
	/** Persist this workbench projection in plugin data. Defaults to false. */
	persistence?: boolean
	initial?: T[] | (() => T[])
...
export class WorkbenchCollectionService {
	...
	constructor(
		public ctx: Context,
		_cfg: unknown,
		private readonly events: Pick<WorkbenchEventsService, 'registerResourceFor'>,
	) {}
	...
	collectionFor<T extends SignalDbItem>(
		...
		if (!name) throw new Error('[workbench] collection(): name required')
		...
		void managed
			...
			.catch((error) => {
				owner.logger.error('workbench collection initialization failed', {
					pluginName,
					collection: name,
					error,
				})
		...
		owner.effects.defer(() => {
			...
			void managed.dispose().catch((error) => {
				owner.logger.warn('workbench collection cleanup failed', {
					pluginName,
					collection: name,
					error,
				})
	...
	private ensureStream(pluginName: string, collection: string, owner: Context) {
		...
		const dispose = this.events.registerResourceFor(
			owner,
			...
			(channel) => {
				...
				if (managed) {
					void managed
						...
						.catch((error) => {
							owner.logger.error('workbench collection initial stream snapshot failed', {
								pluginName,
	...
	broadcast(
		...
		for (const channel of this.channelsFor(pluginName, payload.collection)) {
			try {
				...
				this.ownersByPlugin.get(pluginName)?.logger.warn('workbench collection broadcast failed', {
					pluginName,
...
class ManagedSignalDbCollection<T extends SignalDbItem> {
	...
	constructor(
		...
		private readonly owner: WorkbenchCollectionService,
	...
	async dispose(): Promise<void> {
		try {
			...
			this.ctx.logger.warn('workbench collection cleanup after initialization failure', {
		...
		try {
			...
			this.ctx.logger.warn('workbench collection SignalDB cleanup failed', {
				pluginName: this.options.pluginName,
				collection: this.options.name,
				error,
			})
	...
	private assertReady() {
		if (!this.initialized || !this.collection) {
			throw new Error(
				`[workbench] collection "${this.options.name}" is not ready; await collection.ready() first`,
			)
		}
	}
	...
	private emit(event: SignalDbSyncEvent<T>) {
		for (const listener of this.listeners) {
			try {
				...
				this.ctx.logger.error('workbench collection listener failed', {
					pluginName: this.options.pluginName,
					collection: this.options.name,
					event: event.type,
					error,
	...
	watch(listener: (event: SignalDbSyncEvent<T>) => void): () => void {
		...
		if (this.initialized) {
			...
			void this.ready()
				...
				.catch((error) => {
					this.ctx.logger.error('workbench collection initial listener snapshot failed', {
						pluginName: this.options.pluginName,
						collection: this.options.name,
						error,
					})
