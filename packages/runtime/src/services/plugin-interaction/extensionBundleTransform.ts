import { extensionVendorPackages } from '../../web/plugin-ui/vendors'

export function toBrowserBundleResolve<
	T extends { conditions?: readonly string[] | string[] | undefined },
>(resolve: T): T {
	const conditions = Array.isArray(resolve.conditions) ? resolve.conditions : null
	if (!conditions) return resolve

	const filtered = conditions.filter(
		(c) => c !== '@pluxel/source' && c !== 'source' && c !== '@pluxel/runtime',
	)
	if (filtered.length === conditions.length) return resolve

	return { ...resolve, conditions: filtered } as T
}

export function looksLikeBrokenExtensionBundle(code: string): boolean {
	// Heuristic checks to reject obviously broken browser modules early.
	// This is intentionally conservative: false positives are better handled by fixing the bundler pipeline.
	...
	// Must be an ES module with at least one export.
	if (!/\bexport\s+/.test(code) && !/\bexport\{/.test(code)) {
		return true
	}
	// Browser module must not include Node-only helpers.
	if (/from\s+["']node:module["']/.test(head) || /createRequire\(/.test(head)) return true
	if (/import\s+["']react\/jsx-runtime["'];?/.test(head)) return true
	// Browser has no process; `process.env` indicates an SSR-ish output slipped in.
