...
export function filterPackageRows(rows: PackageRow[], term: string): PackageRow[] {
	if (!term) return rows
	const normalized = term.trim().toLowerCase()
	if (!normalized) return rows
	return rows.filter((row) => {
		if (row.name.toLowerCase().includes(normalized)) return true
		if (row.pluginNames.some((plugin) => plugin.toLowerCase().includes(normalized))) return true
		if ((row.version ?? '').toLowerCase().includes(normalized)) return true
		if ((row.tag ?? '').toLowerCase().includes(normalized)) return true
		return false
	})
}
