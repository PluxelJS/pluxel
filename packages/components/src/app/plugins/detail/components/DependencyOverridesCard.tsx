...
export function DependencyOverridesCard() {
	...
	const load = useCallback(async () => {
		...
		try {
			...
			setState(Array.isArray(deps) ? deps : [])
		} catch (error) {
			if (!mountedRef.current) return
			setState([])
			notify({
				...
				children: (
					<Box>
						<Text size="sm" c="dimmed">
							为 <Text component="span" fw={600}>{baseName}</Text> 创建新的 forkId。
						</Text>
						<TextInput
							autoFocus
							mt="sm"
		...
		<Paper withBorder radius="md" p="sm" shadow="xs">
			<Group justify="space-between" align="center" mb="xs">
				...
				<Tooltip label={loading ? '加载中…' : '刷新'} withArrow>
					<ActionIcon size="sm" variant="subtle" onClick={() => void triggerRefresh()} disabled={loading}>
						<IconRefresh size={14} />
					</ActionIcon>
				</Tooltip>
			</Group>
				...
				<Text size="sm" c="dimmed">
					...
					<Stack gap="sm">
						{rows.map((row, idx) => {
							...
							const currentForkValue = row.kind === 'forkable' ? row.selected ?? row.effective ?? null : null
							...
							const handleForkChange = async (value: string | null) => {
								try {
									...
									// Selecting the original token resets override.
									...
									await triggerRefresh()
									notify({ title: '已更新', message: `${row.token} → ${value ?? '默认'}`, color: 'green' })
								} catch (error) {
									notify({
									title: '更新失败',
									message: rpcErrorMessage(error, '操作失败'),
									...
									{row.kind === 'base' ? (
										<Stack gap={6}>
											...
											<Text size="xs" c="dimmed">
												当前注入：{row.effective}；全局默认：{row.baseProvider ?? '未设置'}（覆盖为空则使用全局默认）
											</Text>
