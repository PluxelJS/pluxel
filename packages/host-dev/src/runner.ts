...
export function collectViteSsrImportFiles(server: ViteDevServer, entry: string): Set<string> {
	...
	while (queue.length > 0) {
		...
		if (
			module.file &&
			[...getViteSsrModuleRunnerState(server).externalModules].some((registration) =>
				registration.modules.has(normalizePath(module.file!)),
			)
		)
			continue
...
function installViteSsrModuleExternalizer(
	...
	if (!transport || typeof transport !== 'object') {
		throw new TypeError('[host-dev/vite] Vite SSR ModuleRunner transport is unavailable')
	}
	...
	if (typeof invoke !== 'function') {
		throw new TypeError('[host-dev/vite] Vite SSR ModuleRunner transport cannot be invoked')
	}
