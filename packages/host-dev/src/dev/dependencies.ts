...
} from '@pluxel/management/internal/api/usecases/pluginDependencies'
...
export function createDevDependencies(
	...
	const resolveRequirement = (requirement: DevPluginRequirement) => {
		...
		const catalog = requirePluginHostCoordinator(ctx).catalogSnapshot()
	...
	return Object.freeze({
		inspect: (consumer) =>
			scope.run(async () => {
				try {
					return {
						ok: true,
						items: await inspectPluginConsumerRequirements(ctx, resolveTarget(consumer), {
		...
		clearDefault: (requirement) =>
			scope.run(() =>
				setPluginProviderDefault(ctx, resolveRequirement(requirement), null, {
					signal: scope.controller.signal,
				}),
			),
		setOverride: (input) =>
