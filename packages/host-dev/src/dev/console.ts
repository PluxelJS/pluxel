...
import { applyLifecycleCommands } from '@pluxel/management/internal/api/usecases/pluginStatus'
...
import { createDevForks } from './forks'
import { createDevDependencies } from './dependencies'
...
export function createDevConsoleScope(
	...
	const lifecycle = (target: DevPluginTarget, command: 'start' | 'stop' | 'restart') =>
		scope.run(async () => {
			const result = await applyLifecycleCommands(
				ctx,
	...
	let workbench: Promise<DevConsole['workbench']> | undefined
	const loadWorkbench = () =>
		(workbench ??= import('./workbench').then(({ createDevWorkbench }) =>
			createDevWorkbench(ctx, scope, resolveTarget),
		))
	let logs: Promise<DevConsole['logs']> | undefined
	const loadLogs = () =>
		(logs ??= import('./logs').then(({ createDevLogs }) =>
			createDevLogs(ctx, scope, resolveTarget),
		))
	const dev: DevConsole = Object.freeze({
		plugins: Object.freeze({
			list: () =>
				scope.run(async () => {
					const overview = await pluginStatusOverview(ctx, admission)
					return overview.plugins
				}),
			status: (target: DevPluginTarget) =>
				scope.run(() => pluginStatus(ctx, resolveTarget(target), admission)),
		...
		config: Object.freeze({
			...
			describe: (target: DevPluginTarget) =>
				scope.run(() => pluginConfigPresentation(ctx, resolveTarget(target), admission)),
			...
			) => scope.run(() => pluginConfigPatchField(ctx, resolveTarget(target), input, admission)),
		...
		commands: Object.freeze({
			...
			execute: (
				...
				scope.run(() =>
					ctx.require(Commands).execute(name, input, {
						...context,
		...
		http: Object.freeze({
			...
			fetch: (input: Request | URL | string, init?: RequestInit) =>
				scope.run(async () => {
					...
					const http = options.fetch ? undefined : await import('@pluxel/services/http')
					const response = await (options.fetch
						? options.fetch(request)
						: resolveContextCapability(ctx, http!.HttpServer).fetch(request))
		...
		workbench: Object.freeze<DevConsole['workbench']>({
			list: (input) =>
				scope.run(async () => {
					const service = await loadWorkbench()
		...
		logs: Object.freeze<DevConsole['logs']>({
			mark: (input) =>
				scope.run(async () => {
					const service = await loadLogs()
					return service.mark(input)
				}),
			tail: (input) =>
				scope.run(async () => {
					const service = await loadLogs()
					return service.tail(input)
				}),
			read: (input) =>
				scope.run(async () => {
					const service = await loadLogs()
