...
import { matchesLogFilter, type LogStreamMeta } from '@pluxel/logging/protocol'
import {
	DevConsoleError,
	type DevConsole,
...
export function createDevLogs(
	...
	const resolve = (streamId = 'default') => {
		...
		const configured = Object.values(logging.resolved.sinks).find(
			(sink): sink is import('@pluxel/logging/internal').RuntimeStoreSinkInput =>
				sink.kind === 'store' &&
	...
	const filterOf = (options: {
		...
		filter?: import('@pluxel/logging/protocol').LogFilter
	}) => {
