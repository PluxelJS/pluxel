...
import { pluginNodeAddressEqual, type Context } from '@pluxel/core'
import { requireContextRuntimeLogging } from '../../../logging/internal'
import type { RuntimeLogStore } from '../../../logging/index'
...
export function logsIndex(ctx: Context): LogsIndexOutput {
	return Object.freeze({
		streams: Object.freeze(
			requireContextRuntimeLogging(ctx)
				.stores.list()
...
function resolveStream(ctx: Context, input?: string): ResolvedLogStream {
	const stores = requireContextRuntimeLogging(ctx).stores
...
export function logsMeta(ctx: Context, input: LogsMetaInput = {}): LogsMetaOutput {
	const resolved = resolveStream(ctx, input.streamId)
	return { ...resolved.store.meta(), streamId: resolved.streamId }
}

export function logsRange(ctx: Context, input: LogsRangeInput): LogsRangeOutput {
	const resolved = resolveStream(ctx, input.streamId)
 ...
 */
export function logsLatest(ctx: Context, input: LogsLatestInput = {}): LogsLatestOutput {
	const resolved = resolveStream(ctx, input.streamId)
 ...
 */
export async function logsWaitFor(
	ctx: Context,
	input: LogsWaitInput = {},
): Promise<LogsLatestOutput> {
	const first = logsLatest(ctx, input)
