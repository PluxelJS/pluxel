...
import type { NodeModuleDeclaration } from '../node/declaration'
import { NodeModules } from '../node/token'
import { WorkerThreadPool } from './pool'
import {
	WorkerTaskError,
...
export class WorkerTaskService {
	...
	constructor(
		...
		this.shared = shared ?? this.state!
	}

	forOwner(owner: CoreContext): WorkerTaskService {
		return owner === this.ctx ? this : new WorkerTaskService(owner, undefined, this.shared)
	}
