...
import { WorkerThreadPool, type WorkerThreadFactory } from '../../src/workers/pool.ts'
...
describe('WorkerThreadPool', () => {
	...
	it('rejects an aborted caller immediately but retains capacity until termination settles', async () => {
		...
		void execution.settled.then((): void => void (resourceSettled = true))
		await Promise.resolve()
	...
	it('waits for detached idle retirement during shutdown', async () => {
		...
		let destroyed = false
		const destroying = pool.destroy().then((): void => void (destroyed = true))
		await Promise.resolve()
