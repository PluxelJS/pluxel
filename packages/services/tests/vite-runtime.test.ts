...
describe('static Vite runtime', () => {
	it('consumes package-root lowering through Host and service composition', async () => {
		await using fixture = await createDiskFixture(
			{ 'pnpm-workspace.yaml': 'packages: []\n' },
			{ tempDir: resolve(workspaceRoot, 'packages/services/tests') },
		)
		...
		await fixture.writeFile(
			...
			[
				...
				'  envBindings: [',
				"    envBinding(ConfiguredPlugin, { config: { schema: ConfiguredPluginConfig, mapping: { label: 'VITE_STATIC_LABEL' } } }),",
				'  ],',
		...
		await expect(
			execFileAsync(process.execPath, ['--import', 'tsx', runner], {
				cwd: resolve(workspaceRoot, 'packages/services'),
				env: {
					...process.env,
					CI: '1',
