...
import {
	...
	type ServiceInternalTestHost,
} from '@pluxel/services/internal/test'
import { afterEach, describe, expect, it, vi } from 'vitest'
import { PluginCatalogLayoutService } from '../../src/management/services/management/PluginCatalogLayoutService.ts'
import {
	automaticCatalogGroups,
	catalogFamilies,
	type PluginCatalogLayoutEntry,
} from '../../src/management/services/management/catalog-groups.ts'

const hosts: ServiceInternalTestHost[] = []
afterEach(async () => {
	vi.restoreAllMocks()
...
describe('editable group document', () => {
	it('does not allocate management or write defaults just to read a catalog', async () => {
		const disabled = await createServiceInternalTestHost({ workbench: false })
		hosts.push(disabled)
