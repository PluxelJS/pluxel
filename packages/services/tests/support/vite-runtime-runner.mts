...
assert.ok(capturedHost, errors.join('\n'))
const initialStatus = await capturedHost.status()
const address = initialStatus.statuses.find(
	(plugin) => plugin.address.definition.exportName === 'ViteStatic',
)?.address
assert.ok(address, 'startup catalog is empty')
const configuredAddress = initialStatus.statuses.find(
	(plugin) => plugin.address.definition.exportName === 'ConfiguredPlugin',
)?.address
assert.ok(configuredAddress, 'configured plugin is absent from the startup catalog')
const partOwnerAddress = initialStatus.statuses.find(
	(plugin) => plugin.address.definition.exportName === 'PartOwner',
)?.address
assert.ok(partOwnerAddress, 'Part owner is absent from the startup catalog')
const partProviderAddress = initialStatus.statuses.find(
	(plugin) => plugin.address.definition.exportName === 'PartProvider',
)?.address
assert.ok(partProviderAddress, 'Part provider is absent from the startup catalog')
const builtAddress = initialStatus.statuses.find(
	(plugin) => plugin.address.definition.exportName === 'BuiltStatic',
)?.address
...
try {
	...
	assert.ok((sourceAttempt?.sequence ?? 0) > (appliedStatus?.recentUpdate?.batch.sequence ?? 0))
	...
	const finalStatusOverview = await readHostPluginStatusOverview(replacementHost.ctx)
	...
	const statusBeforeUnlink = await readHostPluginStatusOverview(serializedHost.ctx)
	try {
		...
		const statusesAfterUnlinkOverview = await readHostPluginStatusOverview(serializedHost.ctx)
		const statusesAfterUnlink = statusesAfterUnlinkOverview.statuses
		const unlinkAttempt = readHostRecentUpdates(serializedHost.ctx)?.latestUpdate()
