...
try {
	...
	const expectRejected = async (change: () => Promise<unknown>, marker: string) => {
		const previous = readHostRecentUpdates(host!.ctx)?.latestUpdate()?.sequence
		await change()
		await eventually(() => {
			const latest = readHostRecentUpdates(host!.ctx)?.latestUpdate()
			assert.equal(latest?.state, 'settled')
			assert.equal(latest?.outcome, 'retained-previous')
			assert.notEqual(latest?.sequence, previous, marker)
		})
	}
	...
	const evaluationError = readHostRecentUpdates(host!.ctx)?.latestUpdate()?.error
	...
	await healthy('helper-fixed')
	const missingError = readHostRecentUpdates(host!.ctx)?.latestUpdate()?.error
	...
	await healthy('outside-manifest-fixed')
	...
	await healthy('installed-package')
	await writeFile(
		resolve(brokenPackage, 'package.json'),
	...
	while (true) {
		try {
			...
			if (Date.now() >= deadline)
				throw new Error(
					`${String(error)}\nLatest update: ${JSON.stringify(host && readHostRecentUpdates(host.ctx)?.latestUpdate())}\nVite errors:\n${errors.join('\n')}`,
					{ cause: error },
				)
		}
		await delay(50)
