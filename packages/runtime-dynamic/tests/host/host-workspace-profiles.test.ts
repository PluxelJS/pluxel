...
import { bootPlannedLoaderHmrHost, planLoaderHmrHost } from '../../src/hmr/host'
import { createTestHmrHost } from '../support/test-host'

describe('@pluxel/runtime-dynamic/hmr/host snapshot contract', () => {
	...
	it('boots deterministically when snapshot is provided', async () => {
		...
		const res = await createTestHmrHost({
			...
			root: fixture.path,
			storage: {
				configFile: '.pluxel/loader-hmr/config.json',
				seedConfig: false,
			},
			snapshot,
		})
		expect(res.root).toBe(resolve(fixture.path))
		expect(fixture.fs.existsSync(resolve(fixture.path, '.pluxel/loader-hmr/config.dev.json'))).toBe(true)
	}, 15_000)
	...
	it('separates host planning from startup side effects', async () => {
		...
		expect(fixture.fs.existsSync(resolve(fixture.path, '.pluxel/loader-hmr/config.dev.json'))).toBe(false)

		const res = await bootPlannedLoaderHmrHost(plan)
		expect(fixture.fs.existsSync(resolve(fixture.path, '.pluxel/loader-hmr/config.dev.json'))).toBe(true)
