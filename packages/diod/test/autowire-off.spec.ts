...
import { BankUser, SessionManager, ShoppingCart, ShopUser } from './fixtures/user'
...
describe('the constructor of the extended class is injected if target has not constructor', () => {
	it('injects deps into subclass without an explicit constructor', () => {
		...
		expectOk(builder.tryRegisterAndUse(BankUser)).withDependencies([SessionManager])
		expectOk(builder.tryRegisterAndUse(ShopUser)).withDependencies([SessionManager, ShoppingCart])
