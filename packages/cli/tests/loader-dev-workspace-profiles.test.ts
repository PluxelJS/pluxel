...
describe('@pluxel/runtime-loader/dev workspace profiles', () => {
	it('parses config strictly (unknown fields rejected)', async () => {
		...
		const fs = fixture.fs as LoaderDevWorkspaceFs
		expect(() => readLoaderDevConfigV1(resolve(fixture.path, 'pluxel.loader.dev.jsonc'), fs)).toThrow()
	})
	...
	it('returns a friendly error when config file is missing', async () => {
		...
		const configPath = resolve(rootDir, 'pluxel.loader.dev.jsonc')
		const fs = fixture.fs as LoaderDevWorkspaceFs
		const res = await diagnoseLoaderDevWorkspace({ rootDir, configPath, env: {}, fs })
		expect(res.ok).toBe(false)
		if (res.ok !== false) return
		expect(res.errors[0]).toContain('Missing config file')
		expect(res.errors[0]).toContain('pluxel.loader.dev.jsonc')
	})

	it('diagnoseLoaderDevWorkspace resolves enabledEntries, include entries, and watchRoots', async () => {
		await using fixture = await createFixture({
			'pnpm-workspace.yaml': ['packages:', "  - 'packages/*'", ''].join('\n'),
			...
			'packages/plugin-a/package.json': JSON.stringify(
				{
					...
					exports: { '.': { '@pluxel/runtime-loader': './src/index.ts' } },
				},
				null,
				2,
			),
			...
			'packages/plugin-builtin/package.json': JSON.stringify(
				{
					...
					version: '0.0.0',
					type: 'module',
					exports: { '.': { import: './dist/index.mjs', '@pluxel/runtime-loader': './src/index.ts' } },
				},
				null,
				2,
			),
		...
		const cfg: PluxelLoaderDevConfigV1 = {
			version: 1,
			profile: 'dev',
			defaults: {
				roots: 'auto',
		...
		writeLoaderDevConfigV1(configPath, cfg, { headerComment: '', fs })
	...
	it('warns when a selected workspace plugin depends on another unselected workspace plugin', async () => {
		...
		writeLoaderDevConfigV1(
			configPath,
			{
				...
				profile: 'dev',
				defaults: { roots: 'auto' },
				profiles: { dev: { enabled: ['pluxel-plugin-consumer'] } },
			},
			{ headerComment: '', fs },
		)
			...
			'builtin-plugins/graphql/package.json': JSON.stringify(
				{
					...
					version: '0.0.0',
					type: 'module',
					exports: { '.': { '@pluxel/runtime-loader': './src/index.ts', default: './dist/index.js' } },
				},
				null,
				2,
			),
