...
import {
	...
	buildLoaderDevWorkspaceFromScan,
...
type SnapshotState =
	...
	| { status: 'ok'; snapshot: LoaderDevWorkspace; warnings: string[] }

function LoaderDevPromptApp(props: {
	rootDir: string
	configPath: string
	env: Record<string, string | undefined>
	skipPackages: Set<string>
	initialCfg: PluxelLoaderDevConfigV1
	...
	const [cfg, setCfg] = useState<PluxelLoaderDevConfigV1>(props.initialCfg)
	...
	useEffect(() => {
		...
		setModal({
			...
			onConfirm: () => {
				try {
					const repaired = createDefaultLoaderDevConfigV1()
					backupAndRewriteLoaderDevConfigV1(props.configPath, repaired)
					setCfg(repaired)
	...
	const mergedResult = useMemo(() => {
		try {
			const env = { ...props.env, PLUXEL_DEV_PROFILE: activeProfile }
			return { merged: mergeLoaderDevProfile(cfg, env), error: null as string | null }
		} catch (error) {
			return {
				merged: null,
	...
	useEffect(() => {
		...
		async function run() {
			...
			try {
				...
				const rootsExpandedAbs = await resolveLoaderDevRootsExpanded(rootDirAbs, merged.roots)
			...
			async function run() {
				try {
					...
					const omitPackages = merged.builtinPackages?.length
						...
						: undefined
					const snapshotRes = await buildLoaderDevWorkspaceFromScan({
						rootDir: rootDirAbs,
						merged,
	...
	function saveConfig() {
		try {
			writeLoaderDevConfigV1(props.configPath, { ...cfg, profile: activeProfile })
			setDirty(false)
	...
	// Auto-open flows (when invoked via `pluxel dev enabled/builtin` etc).
	useEffect(() => {
			...
			setModal({
				...
				onSubmit: (name) => {
					...
					setCfg((prev) => {
						...
						const cloned = JSON.parse(JSON.stringify(data)) as PluxelLoaderDevConfigV1['profiles'][string]
...
export async function runLoaderDevPromptTui(params: {
	rootDir: string
	configPath: string
	env: Record<string, string | undefined>
	skipPackages: Set<string>
	...
	if (!process.stdout.isTTY || !process.stdin.isTTY) {
		throw new Error('Interactive `pluxel dev` requires a TTY.')
	}

	let initialCfg: PluxelLoaderDevConfigV1
	let initialProfile = 'dev'
	let initialDirty = false
	let initialParseError: string | null = null

	if (!existsSync(params.configPath)) {
		initialCfg = createDefaultLoaderDevConfigV1()
		initialProfile = params.env.PLUXEL_DEV_PROFILE ?? initialCfg.profile
		initialDirty = true
	} else {
		try {
			initialCfg = readLoaderDevConfigV1(params.configPath)
			initialProfile = params.env.PLUXEL_DEV_PROFILE ?? initialCfg.profile
		} catch (e) {
			initialCfg = createDefaultLoaderDevConfigV1()
