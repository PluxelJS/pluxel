...
function resolvePluginEntryAbs(pkgDirAbs: string, manifest: PackageJson): string | null {
	...
	const loaderDevEntry = (dot as any)[PLUXEL_CONDITION_RUNTIME_LOADER]
	if (typeof loaderDevEntry !== 'string' || !loaderDevEntry.trim()) return null

	return resolve(pkgDirAbs, loaderDevEntry)
}
