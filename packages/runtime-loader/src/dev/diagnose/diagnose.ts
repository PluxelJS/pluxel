...
import type { BuiltinsFromDistEntry, LoaderDevWorkspaceSnapshot } from '../snapshot'
import { type PluxelLoaderDevConfigV1, readLoaderDevConfigV1, resolveDefaultLoaderDevConfigPath } from './config'
import {
...
export type DiagnoseWorkspaceInput = {
	 ...
	 * so workspace profiles should not also load their `@pluxel/runtime-loader` source entries
...
export type WorkspaceSnapshot = LoaderDevWorkspaceSnapshot & { discovered: DiscoveredPlugin[] }
...
export function mergeLoaderDevProfile(
	cfg: PluxelLoaderDevConfigV1,
	env?: Record<string, string | undefined>,
): MergedProfile {
	const activeProfile = env?.PLUXEL_DEV_PROFILE ?? cfg.profile
	const profile = cfg.profiles[activeProfile]
	if (!profile) {
		throw new Error(
			`[loader-dev] Unknown profile "${activeProfile}". Available: ${Object.keys(cfg.profiles).join(', ')}`,
		)
	}
...
export async function resolveLoaderDevRootsExpanded(
	rootDir: string,
	roots: 'auto' | string[],
	fs: WorkspaceFs = nodeWorkspaceFs,
): Promise<string[]> {
...
export async function buildWorkspaceSnapshotFromScan(params: {
	...
	for (const name of effectiveEnabled) {
		...
		if (!plugin) {
			errors.push(`[loader-dev] Enabled plugin not found: ${name}`)
			continue
		}
		const entryAbs = toPosix(resolve(rootDirAbs, plugin.entry))
		if (!fs.existsSync(entryAbs)) {
			errors.push(`[loader-dev] Plugin entry missing on disk: ${name} -> ${plugin.entry}`)
			continue
		}
	...
	if (skippedEnabled.length > 0) {
		warnings.push(
			`[loader-dev] Skipped ${skippedEnabled.length} enabled package(s) because they are provided by builtins: ${skippedEnabled.join(
				', ',
			)}`,
		)
	}

	{
		...
		if (missingEdges.length > 0) {
			...
			warnings.push(
				`[loader-dev] ${missingEdges.length} selected plugin package(s) depend on other plugin packages that are not selected in this profile. Consider adding them to profile.enabled to avoid MissingDependency when their plugins are enabled at runtime.`,
			)
			for (const edge of shown) {
				warnings.push(`[loader-dev] Missing profile packages: ${edge.from} -> ${edge.missing.join(', ')}`)
			}
			if (missingEdges.length > maxEdges) {
				warnings.push(`[loader-dev] …and ${missingEdges.length - maxEdges} more missing-deps edge(s).`)
			}
		}
	}
	{
		...
		if (missingEdges.length > 0) {
			const maxEdges = 20
			warnings.push(
				`[loader-dev] ${missingEdges.length} selected plugin package(s) depend on managed (node_modules) plugin packages that are not declared in the workspace root package.json. If you rely on PackageService auto-load, add them to root deps (or load them manually / provide as builtins).`,
			)
			for (const edge of missingEdges.slice(0, maxEdges)) {
				warnings.push(
					`[loader-dev] Missing root deps (managed plugins): ${edge.from} -> ${edge.missing.join(', ')}`,
				)
			}
			if (missingEdges.length > maxEdges) {
				warnings.push(
					`[loader-dev] …and ${missingEdges.length - maxEdges} more missing managed-plugin edge(s).`,
				)
			}
		}
	}
...
export async function diagnoseWorkspace(
	...
	const fs = input.fs ?? nodeLoaderDevWorkspaceFs

	if (!fs.existsSync(configPathAbs))
		return { ok: false, errors: [`Missing config file: ${configPathAbs}`] }

	let cfg: PluxelLoaderDevConfigV1
	try {
		cfg = readLoaderDevConfigV1(configPathAbs, fs)
	} catch (error) {
	...
	try {
		merged = mergeLoaderDevProfile(cfg, input.env)
	} catch (error) {
		return { ok: false, errors: [error instanceof Error ? error.message : String(error)] }
	}
	...
	try {
		rootsExpandedAbs = await resolveLoaderDevRootsExpanded(rootDirAbs, merged.roots, fs)
	} catch (error) {
		return { ok: false, errors: [error instanceof Error ? error.message : String(error)] }
	}
	...
	for (const pkgName of builtinPkgs) {
		...
		if (!pkg)
			return {
				ok: false,
				errors: [`[loader-dev] Builtin package not found in workspace: ${pkgName}`],
		...
		if (!rel) {
			return {
				ok: false,
				errors: [
					`[loader-dev] Builtin package missing dist .mjs export entry (check package.json exports): ${pkgName}`,
				],
				discovered: base.snapshot.discovered,
			}
		}
		...
		if (!fs.existsSync(entryAbs)) {
			return {
				ok: false,
				errors: [`[loader-dev] Builtin dist entry missing on disk for ${pkgName}: ${entryAbs}`],
				discovered: base.snapshot.discovered,
			}
		}
...
export function resolveLoaderDevConfigPathFromCwd(cwd = process.cwd()) {
	return resolveDefaultLoaderDevConfigPath(cwd)
}
