 ...
 */
export type LoaderDevWorkspaceSnapshot = {
...
function assertStringArray(value: unknown, label: string): asserts value is string[] {
	if (!Array.isArray(value)) throw new Error(`[loader-dev] Invalid snapshot: ${label} missing.`)
	if (value.some((x) => typeof x !== 'string')) {
		throw new Error(`[loader-dev] Invalid snapshot: ${label} must be string[].`)
	}
}

export function assertLoaderDevWorkspace(
	snapshot: unknown,
): asserts snapshot is LoaderDevWorkspaceSnapshot {
	if (!snapshot || typeof snapshot !== 'object' || Array.isArray(snapshot)) {
		throw new Error('[loader-dev] Invalid snapshot: expected object.')
	}
	const s = snapshot as Record<string, unknown>
	if (typeof s.activeProfile !== 'string' || !s.activeProfile.trim()) {
		throw new Error('[loader-dev] Invalid snapshot: activeProfile missing.')
	}
	...
	if (s.builtinsFromDist !== undefined) {
		if (!Array.isArray(s.builtinsFromDist)) {
			throw new TypeError('[loader-dev] Invalid snapshot: builtinsFromDist must be an array.')
		}
		for (const raw of s.builtinsFromDist) {
			if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
				throw new TypeError('[loader-dev] Invalid snapshot: builtinsFromDist must contain objects.')
			}
			const o = raw as Record<string, unknown>
			if (typeof o.packageName !== 'string' || !o.packageName.trim()) {
				throw new Error('[loader-dev] Invalid snapshot: builtinsFromDist[].packageName must be a string.')
			}
			if (typeof o.entry !== 'string' || !o.entry.trim()) {
				throw new Error('[loader-dev] Invalid snapshot: builtinsFromDist[].entry must be a string.')
			}
		}
	}
}
