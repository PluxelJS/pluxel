...
import {
	PLUXEL_CONDITION_RUNTIME_LOADER,
...
export const BASE_LOADER_HMR_RESOLVE_CONDITIONS = [PLUXEL_CONDITION_RUNTIME_LOADER, PLUXEL_CONDITION_SOURCE] as const
...
export function buildLoaderHmrViteConfig(opts: HmrViteConfigOptions): InlineConfig {
	...
	const clientConditions = ssrConditions.filter(
		(c) => c !== PLUXEL_CONDITION_RUNTIME_LOADER && c !== PLUXEL_CONDITION_SOURCE,
	)
