...
export interface LoaderDevUiSourceDeclaration {
	/** Authoring/dev-only declaration. Runtime never consumes this path directly. */
	entryPath: string
}

export interface LoaderDevUiModuleDeclaration {
	/**
	 * Loader dev bridge entry.
	 *
	 * Plugin code keeps calling `ui(...).bind(ctx)` so build-time tooling can recognize and rewrite
	 * this declaration if needed. In dev, the loader consumes the source declaration directly; outside dev
...
function normalizeUiConfig(input: string | LoaderDevUiSourceDeclaration): LoaderDevUiSourceDeclaration {
	const entryPath =
		typeof input === 'string' ? String(input).trim() : String(input.entryPath ?? '').trim()
	return { entryPath }
}

export function ui(input: string | LoaderDevUiSourceDeclaration): LoaderDevUiModuleDeclaration {
	const config = normalizeUiConfig(input)
	if (!config.entryPath) throw new Error('[pluxel/runtime-loader/plugin] ui(): entryPath required')
...
export type LoaderDevWorkerMode = 'dev' | 'fallback'

export type LoaderDevWorkerSnapshot = {
	mode: LoaderDevWorkerMode
	url: string | null
}

export type LoaderDevWorkerDeclarationOptions = {
	external?: string[]
	fallback?: string | null
}

export type LoaderDevWorkerBindOptions = LoaderDevWorkerDeclarationOptions & {
	onUpdate?: (snapshot: LoaderDevWorkerSnapshot) => void | Promise<void>
	onError?: (error: unknown) => void
}

export interface LoaderDevWorkerBinding {
	snapshot(): LoaderDevWorkerSnapshot
	dispose(): Promise<void>
}

export interface LoaderDevWorkerDeclaration {
	readonly entryPath: string
	bind(ctx: Context, options?: LoaderDevWorkerBindOptions): Promise<LoaderDevWorkerBinding>
}
...
export function worker(
	entryPath: string,
	defaults: LoaderDevWorkerDeclarationOptions = {},
): LoaderDevWorkerDeclaration {
	const normalizedEntry = String(entryPath ?? '').trim()
	if (!normalizedEntry) {
		throw new Error('[pluxel/runtime-loader/plugin] worker(): entryPath required')
	}
	...
	return {
		...
		async bind(
			ctx: Context,
			options: LoaderDevWorkerBindOptions = {},
		): Promise<LoaderDevWorkerBinding> {
			const external = options.external ? [...options.external] : defaultExternal
			const fallback = options.fallback ?? defaultFallback
			const onUpdate = options.onUpdate
			const onError = options.onError
			const state: LoaderDevWorkerSnapshot = {
				mode: 'fallback',
