...
import type { ManagementArtifactStore } from '@pluxel/runtime/internal'
...
const pluginBuildMocks = vi.hoisted(() => ({
	...
	resolveManagementFederationShared: vi.fn(() => ({ signature: 'shared-signature' })),
	resolveManagementUiBuildSignature: vi.fn(() => 'ui-build-signature'),
}))

vi.mock('@pluxel/rolldown/vite/management-ui', () => ({
	buildManagementUiRemote: pluginBuildMocks.buildManagementUiRemote,
	resolveManagementFederationShared: pluginBuildMocks.resolveManagementFederationShared,
	resolveManagementUiBuildSignature: pluginBuildMocks.resolveManagementUiBuildSignature,
}))

import { ManagementCompilerService } from '../src/management/ManagementCompilerService'
...
describe('ManagementCompilerService', () => {
	beforeEach(() => {
		pluginBuildMocks.buildManagementUiRemote.mockClear()
		pluginBuildMocks.resolveManagementFederationShared.mockClear()
		pluginBuildMocks.resolveManagementUiBuildSignature.mockClear()
	})
	...
	it('resolves relative UI entries from the declaring plugin source file', async () => {
		...
		const committed: Parameters<ManagementArtifactStore['commitCompiledModule']>[0][] = []
		...
		const service = new ManagementCompilerService(
			...
			{ store, enabled: true },
			{
				...
				vite: {
					...
					resolve: {
						alias: {
							'@generated/management-ui': fixture.getPath('generated/management-ui.ts'),
						},
					},
				},
			},
		...
		expect(artifactRoots[0]).toContain('.pluxel/management/PluginWithUI')
		expect(pluginBuildMocks.buildManagementUiRemote).toHaveBeenCalledWith(
		...
		expect(pluginBuildMocks.resolveManagementUiBuildSignature).toHaveBeenCalledWith(
			expect.objectContaining({
				plugins: [{ name: 'test:ui-transform' }],
			}),
			undefined,
		)
		expect(pluginBuildMocks.buildManagementUiRemote).toHaveBeenCalledTimes(1)
