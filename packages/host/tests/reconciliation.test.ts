...
import {
	...
	type HostStateCoordinatorStore,
...
function state(input: Partial<HostStateSnapshot> = {}): HostStateSnapshot {
	return Object.freeze({
...
describe('Host Plugin reconciliation', () => {
	it('removes batched node policy through the reverse override index', () => {
		...
		const next = applyHostStatePatch(
			snapshot,
			hostStatePatch(
	...
	it('infers an abstract default deterministically and persists it before graph apply', () => {
		...
		const catalog = createPluginCatalogSnapshot(1, [
	...
	it('rejects optional edges that would bind through an abstract provider role', () => {
		...
		expect(() =>
			createPluginCatalogSnapshot(1, [{ candidate: candidate(Consumer, { optional: [Token] }) }]),
		).not.toThrow()
	})
	...
	it('replaces one definition operation for all materialized variants', () => {
		...
		const firstCatalog = createPluginCatalogSnapshot(1, [{ candidate: first }])
		const firstPlan = reconcilePluginGraph({
			catalog: firstCatalog,
			runtimeState,
			runtimeStateRevision: 1,
		})
		const nextCatalog = createPluginCatalogSnapshot(2, [{ candidate: next }])
	...
	it('rejects a live forkability contraction but reports it as blocked on cold boot', () => {
		...
		const previous = createPluginCatalogSnapshot(1, [
			{ candidate: candidate(Provider, { forkable: true }) },
		])
	...
	it('blocks a concrete-token provider default without sending it to Core', () => {
		...
		const catalog = createPluginCatalogSnapshot(1, [{ candidate: candidate(Provider) }])
		const plan = reconcilePluginGraph({
			catalog,
			runtimeState: state({
				autoStart: [provider],
	...
	it('commits source removal without rewriting explicit bindings and recovers on reappearance', () => {
		...
		const unlinked = createPluginCatalogSnapshot(2, [{ candidate: consumerCandidate }])
		...
		const restored = createPluginCatalogSnapshot(3, [
			{ candidate: providerCandidate },
			{ candidate: consumerCandidate },
		])
	...
	it('propagates a blocked dependency chain with one requirement scan per consumer', () => {
		...
		const catalog = createPluginCatalogSnapshot(
			1,
...
describe('Host Plugin graph publication', () => {
	it('clears process-local session intent when reconciling a cold boot', async () => {
		...
		const catalog = createPluginCatalogSnapshot(1, [{ candidate: candidate(Plugin) }])
	...
	it('rejects a live invalid consumer addition without publishing its catalog', async () => {
		...
		const provider = node(Provider)
		const consumer = node(Consumer)
		const previous = createPluginCatalogSnapshot(1, [{ candidate: candidate(Provider) }])
		const next = createPluginCatalogSnapshot(2, [
	...
	it('publishes the coordinator catalog in the graph-confirmation stack', async () => {
		...
		const events: string[] = []
		let coordinator: PluginHostCoordinator<string>
		...
		coordinator = new PluginHostCoordinator(store, driver)
	...
	it('keeps the coordinator catalog published after a post-PONR Core failure', async () => {
		...
		const firstCatalog = createPluginCatalogSnapshot(1, [{ candidate: candidate(Plugin) }])
		const runtimeState = state({ autoStart: [node(Plugin)] })
		const afterPonr = new PluginHostCoordinator(
	...
	it('rejects definition role changes across an absent intermediate catalog', async () => {
		...
		for (const [first, conflicting] of [
			[
				createPluginCatalogSnapshot(1, [{ candidate: candidate(Provider, { provides: Token }) }]),
				createPluginCatalogSnapshot(3, [{ candidate: candidate(Token) }]),
			],
			[
				createPluginCatalogSnapshot(1, [{ candidate: candidate(Token) }]),
				createPluginCatalogSnapshot(3, [{ candidate: candidate(Provider, { provides: Token }) }]),
			],
		] as const) {
			const coordinator = new PluginHostCoordinator(
				memoryStateStore(state()),
			...
			const absent = createPluginCatalogSnapshot(2, [])
	...
	it('restarts an applied node without scanning or reconciling the catalog', async () => {
		...
		const base = createPluginCatalogSnapshot(1, [{ candidate: candidate(Plugin) }])
	...
	it('rejects restart for an unmaterialized node with a stable typed error', async () => {
		...
		await coordinator.reconcileStartup(
			createPluginCatalogSnapshot(1, [{ candidate: candidate(Plugin) }]),
		)
...
describe('Host pinned mutation admission', () => {
	it('rejects a queued first auto-start mutation after the catalog definition is unlinked', async () => {
		...
		const unlink = coordinator.updateCatalog(createPluginCatalogSnapshot(2, []))
		const autoStart = coordinator.updateRuntimeState(
			hostStatePatch({ type: 'set-auto-start', node: address, autoStart: true }),
		)
		await unlink
	...
	it('rejects a queued first fork after definition unlink or forkability contraction', async () => {
		...
		const forkable = createPluginCatalogSnapshot(1, [
			{ candidate: candidate(Provider, { forkable: true }) },
		])

		for (const next of [
			createPluginCatalogSnapshot(2, []),
			createPluginCatalogSnapshot(2, [{ candidate: candidate(Provider, { forkable: false }) }]),
		]) {
			const store = memoryStateStore(state())
			const coordinator = new PluginHostCoordinator(
				store,
			...
			const ensureFork = coordinator.updateRuntimeState(
				hostStatePatch({ type: 'ensure-fork', definition: Provider, forkId: 'queued' }),
			)
			await catalogUpdate
	...
	it('rejects fork removal against an override inserted earlier in the same host queue', async () => {
		...
		await expect(removeFork).rejects.toSatisfy((error: unknown) => {
			if (!(error instanceof HostStateMutationRejectedError)) return false
			return (
	...
	it('does not let a later override interleave with fork metadata cleanup', async () => {
		...
		const removeFork = coordinator.runExclusive('test-exclusive-remove', async (session) => {
			...
			await session.update({
				statePatch: hostStatePatch({ type: 'set-auto-start', node: fork, autoStart: false }),
			})
			metadataCleanups++
			enteredMetadata()
...
function memoryStateStore(initial: HostStateSnapshot): HostStateCoordinatorStore {
