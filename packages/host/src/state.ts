...
export type HostStatePatchOperation =
...
export type HostStatePatch = Readonly<{
	operations: readonly HostStatePatchOperation[]
}>

export const EMPTY_HOST_STATE_PATCH: HostStatePatch = Object.freeze({
	operations: Object.freeze([]),
})

export function hostStatePatch(...operations: readonly HostStatePatchOperation[]): HostStatePatch {
	return Object.freeze({ operations: Object.freeze([...operations]) })
}

export function appendHostStatePatch(
	left: HostStatePatch | undefined,
	right: HostStatePatch | undefined,
): HostStatePatch | undefined {
	if (!left || left.operations.length === 0) return right
	if (!right || right.operations.length === 0) return left
	return hostStatePatch(...left.operations, ...right.operations)
}

export function applyHostStatePatch(
...
export function hostStateEqual(left: HostStateSnapshot, right: HostStateSnapshot): boolean {
	return (
		orderedKeysEqual(
