...
import type { HostApplication } from './host'
...
>(
	...
	if (typeof plugin !== 'function') {
		throw new TypeError('[host] Config environment binding Plugin must be a constructor')
	}
	if (!isConfigEnvironmentSchema(schema)) {
		throw new TypeError('[host] Config environment binding schema must be a Valibot schema')
	}
...
/** @internal Resolve and decode bootstrap declarations before the Host graph is created. */
export function resolveConfigEnvironmentBootstrap(
	application: Pick<HostApplication, 'plugins' | 'configEnvironmentBootstrap'>,
	...
	for (const declaration of declarations) {
		...
		if (!catalogImplementations.has(binding.plugin)) {
			throw new Error(
				'[host] Config environment binding targets a Plugin outside the static application catalog',
			)
		}
		if (boundImplementations.has(binding.plugin)) {
			const candidate = consumePluginDefinitionCandidate(binding.plugin)
			throw new Error(
				`[host] Duplicate config environment binding for ${formatPluginNodeReference({ definition: candidate.declaration.address, variant: 'default' })}`,
			)
		}
		...
		if (declaredSchema !== binding.schema) {
			throw new Error(
				`[host] Config environment binding schema does not match the root config schema declared by ${formatPluginNodeReference(owner)}`,
			)
		}
		...
		for (const target of projectedTargets) {
			...
			if (projection.ok === false) {
				throw new Error(
					`[host] Cannot bind ${target.environmentName} to ${formatPluginNodeReference(owner)} config path ${formatPath(target.path)}: ${projection.reason}`,
				)
			}
			targets.push(
				Object.freeze({
...
function readBinding(value: ConfigEnvironmentBinding): ConfigEnvironmentBindingPayload {
	if (!value || typeof value !== 'object') {
		throw new TypeError(
			'[host] configEnvironmentBootstrap entries must be created with bindConfigEnvironment(...)',
		)
	}
...
function freezeMapping(value: unknown, path: readonly string[] = []): FrozenMapping {
	...
	if (!isPlainRecord(value)) {
		throw new TypeError(
			`[host] Config environment mapping at ${formatPath(path)} must be an environment name or object tree`,
		)
	}
	const keys = Reflect.ownKeys(value)
	if (keys.length === 0) {
		throw new Error(
			`[host] Config environment mapping at ${formatPath(path)} must contain at least one binding`,
		)
	}
	const mapping: Record<string, FrozenMapping> = Object.create(null)
	for (const key of keys) {
		if (typeof key !== 'string') {
			throw new TypeError('[host] Config environment mapping keys must be strings')
		}
		const descriptor = Object.getOwnPropertyDescriptor(value, key)
		if (!descriptor || !descriptor.enumerable || !('value' in descriptor)) {
			throw new TypeError(
				`[host] Config environment mapping at ${formatPath([...path, key])} must use enumerable data properties`,
			)
		}
...
function assertEnvironmentName(value: string, path: readonly string[]): void {
	if (!ENVIRONMENT_NAME.test(value)) {
		throw new Error(
			`[host] Config environment name at ${formatPath(path)} must match [A-Z_][A-Z0-9_]*`,
		)
	}
	if (value.startsWith(RESERVED_ENVIRONMENT_PREFIX)) {
		throw new Error(`[host] Config environment name ${value} is reserved for the Pluxel framework`)
	}
}
...
function decodeTargets(
	...
	for (const target of targets) {
		...
		if (previous && previous.transport !== target.transport) {
			throw new Error(
				`[host] Config environment ${target.environmentName} has conflicting derived transports ${previous.transport} and ${target.transport}`,
			)
		}
	...
	for (const environmentName of [...byEnvironment.keys()].sort()) {
		...
		for (const target of group.targets) {
			if ((target.path.length === 0 || target.expectsPlainObject) && !isPlainRecord(decoded)) {
				throw new Error(
					`[host] Config environment ${environmentName} must decode to a plain object for ${formatPluginNodeReference(target.owner)} config path ${formatPath(target.path)}`,
				)
			}
	...
	const expected =
		transport === 'number'
			...
			: transport === 'boolean'
				? 'JSON true or false'
				: 'JSON'
	return new Error(`[host] Config environment ${environmentName} must contain ${expected}`)
}
...
function cloneRecord(value: unknown): Record<string, unknown> {
	if (!isPlainRecord(value)) throw new TypeError('[host] Expected a plain config object')
