...
import type { CorePluginGraphDriver, CorePluginUpdateDraft } from './coordinator'

export function corePluginGraphDriver(ctx: Context): CorePluginGraphDriver<CommitSummary> {
				...
				prepare: () => {
					...
					return {
						async commit(commitOptions): Promise<CommitSummary> {
							...
							if (!summary) {
								throw new Error('[host:reconciliation] Core commit omitted CommitSummary')
							}
							return summary
						},
