...
import { pluginCatalogEntry, type PluginCatalogEntry, type PluginCatalogSnapshot } from './catalog'
import type { HostStatePatch, HostStatePatchOperation } from './state'

export type HostStateMutationRejection =
	| Readonly<{
...
export class HostStateMutationRejectedError extends Error {
	public readonly code: HostStateMutationRejection['code']

	constructor(public readonly issue: HostStateMutationRejection) {
		super(`[host:state] ${issue.message}`)
		this.name = 'HostStateMutationRejectedError'
 ...
 */
export function validateHostStateMutation(
	catalog: PluginCatalogSnapshot,
	pinnedState: HostStateSnapshot,
	patch: HostStatePatch | undefined,
): void {
	if (!patch || patch.operations.length === 0) return
	admitHostStateMutation(catalog, pinnedState, patch)
}

/** Admits and applies one patch while reusing the same pinned-state indexes. */
export function applyHostStateMutation(
	catalog: PluginCatalogSnapshot,
	pinnedState: HostStateSnapshot,
	patch: HostStatePatch | undefined,
): HostStateSnapshot {
	if (!patch || patch.operations.length === 0) return pinnedState
	return snapshotAdmissionState(admitHostStateMutation(catalog, pinnedState, patch))
}

function admitHostStateMutation(
	catalog: PluginCatalogSnapshot,
	pinnedState: HostStateSnapshot,
	patch: HostStatePatch,
): AdmissionState {
...
type AdmissionState = {
	...
	providerDefaults: Map<string, HostProviderDefaultState>
	dependencyOverrides: Map<string, HostDependencyOverrideState>
	overridesByProvider: Map<string, Set<string>>
	overridesByConsumer: Map<string, Set<string>>
}

function createAdmissionState(snapshot: HostStateSnapshot): AdmissionState {
...
function snapshotAdmissionState(state: AdmissionState): HostStateSnapshot {
	return freezeTrustedHostStateSnapshot({
...
function validateOperation(
	...
	operation: HostStatePatchOperation,
): void {
	switch (operation.type) {
		...
		case 'remove-fork': {
			...
			const references = [...(state.overridesByProvider.get(pluginNodeIndexKey(node)) ?? [])]
				...
				.filter((entry): entry is HostDependencyOverrideState => !!entry)
				.map((entry) =>
					Object.freeze({
...
function setOverride(state: AdmissionState, override: HostDependencyOverrideState): void {
...
function reject(issue: HostStateMutationRejection): never {
	throw new HostStateMutationRejectedError(Object.freeze(issue))
}
