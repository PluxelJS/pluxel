...
import { createPluginCatalogSnapshot, reconcilePluginGraph } from '../src/internal'
import { applyHostStatePatch, hostStatePatch } from '../src/state'
import type { HostStateSnapshot } from '../src/policy'
...
const forkProjectionCatalog = createPluginCatalogSnapshot(1, [
...
const state: HostStateSnapshot = Object.freeze({
...
test('HostState indexed mutation', async ({ bench }) => {
	await bench('remove 1,000 consumers from 5,000 overrides', () => {
		const next = applyHostStatePatch(state, removal)
		if (next.dependencyOverrides.length !== consumerCount - removalCount) {
			throw new Error('HostState benchmark produced an invalid result')
		}
...
test('stopped durable fork projection', async ({ bench }) => {
	const projections = forkProjectionSizes.map((size) => {
		const runtimeState: HostStateSnapshot = Object.freeze({
