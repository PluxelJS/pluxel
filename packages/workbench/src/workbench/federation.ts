import { bindWorkbenchManagement, type WorkbenchManagementOperations } from './management.ts'
...
import * as WorkbenchClient from './client.ts'
import {
	openWorkbenchEntry,
	type WorkbenchClientFederatedOpenResult,
	type WorkbenchOpenedViewHandle,
} from './client.ts'
import type { WorkbenchFederatedLayoutEntry, WorkbenchSessionApi } from './client-protocol.ts'
import * as WorkbenchReactInternal from './react-internal.tsx'
import * as WorkbenchReact from './react.tsx'
import {
	readWorkbenchBridgeIdentity,
	readWorkbenchBridgeProvider,
	type WorkbenchBridgeProvider,
} from './react-internal.tsx'
import type {
...
import type { WorkbenchPaneLayoutRenderer } from './ui-pane.tsx'

export { WorkbenchPaneLayoutControlsProvider } from './ui-pane.tsx'
export type {
...
const WORKBENCH_SHARED_IMPLEMENTATIONS = {
	...
	'@pluxel/workbench/client': WorkbenchClient,
	'@pluxel/workbench/react': WorkbenchReact,
	'@pluxel/workbench/internal/react': WorkbenchReactInternal,
...
export type CreateWorkbenchViewHostInput = Readonly<{
	/** Explicitly borrow the authenticated session's unary management operations. Omitted means unavailable. */
	management?: WorkbenchManagementOperations
	locale: string
...
export class WorkbenchViewHostHandle implements Disposable {
	...
	constructor(input: CreateWorkbenchViewHostInput) {
		...
		this.facade = Object.freeze({
			management: input.management ? bindWorkbenchManagement(input.management, assertActive) : null,
			get locale() {
				return getLocale()
			},
			get colorScheme() {
