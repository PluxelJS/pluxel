...
import type { WorkbenchLayoutEntry, WorkbenchSessionApi } from '@pluxel/workbench/client'
import {
	createWorkbenchViewHost,
	type WorkbenchManagementOperations,
...
it('borrows management operations only for the opened View lifetime', async () => {
	let finish!: (value: { publicKey: string }) => void
	const pending = new Promise<{ publicKey: string }>((resolve) => {
		finish = resolve
	})
	const ensureHostKey = vi.fn(() => pending)
	...
	const management = {
		...
		security: {
			...
			vault: {
				unlock: vi.fn(),
				ensureHostKey,
				generateDeployKey: vi.fn(),
				setDeployRecipients: vi.fn(),
			},
		},
		[Symbol.dispose]: sessionDispose,
	} as unknown as WorkbenchManagementOperations
	const host = createWorkbenchViewHost({
		locale: 'en',
		colorScheme: 'light',
		notify() {},
	...
	const cached = host.facade.management!.security.vault
	const detached = cached.ensureHostKey
	const accepted = detached()
	expect(accepted).toBe(pending)
	host[Symbol.dispose]()
	expect(() => cached.ensureHostKey()).toThrow(/closed/i)
	expect(() => detached()).toThrow(/closed/i)
	finish({ publicKey: 'age1-example' })
	await expect(accepted).resolves.toEqual({ publicKey: 'age1-example' })
	expect(ensureHostKey).toHaveBeenCalledTimes(1)
	expect(sessionDispose).not.toHaveBeenCalled()
	const withoutManagement = createWorkbenchViewHost({
		locale: 'en',
		colorScheme: 'light',
		notify() {},
		confirm: async () => true,
	})
	expect(withoutManagement.facade.management).toBeNull()
	withoutManagement[Symbol.dispose]()
})
