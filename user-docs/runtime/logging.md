---
title: 结构化日志
description: 使用 Context logger、稳定属性和 debug topic，交给宿主管理 sink 与策略。
---

# 结构化日志

Plugin 只使用 `ctx.logger`。宿主为整个进程安装一个 logging root，统一负责 console/file/store/OTel sink、routing、动态等级策略和 shutdown。
