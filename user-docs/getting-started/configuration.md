---
title: 配置模型
description: 用一个 Valibot object schema 定义默认值、归一化、校验和宿主配置边界。
---

# 配置模型

每个具体 Plugin 只有一份配置 authority：传给 `this.configs.use()` 的 object schema。类型、默认值、归一化、运行时校验和 Workbench 表单 metadata 都从它派生。
