...
默认 namespace 包含 caller plugin ID；跨插件共享明确 quota contract 时使用 `rates.global.use()`。identity 可为 primitive、tuple 或 primitive plain record，由框架 canonical encode；name、string identity 和 record 字段名必须是 well-formed Unicode（不含未配对 surrogate）。不需要手工拼接 key，也不能放入 credential 等 secret。
