---
title: 跨仓库源码开发
description: 在保持 Git、workspace 与 lockfile 独立的前提下消费本地 Pluxel 源码。
---

# 跨仓库源码开发

当应用要同时修改尚未发布的 Pluxel 或其他独立仓库时，用 `pluxel source` 管理开发期 resolution。它不会把 checkout 合并进当前 workspace，也不会接管运行期 Plugin 安装。
