...
`pluxel build` 从 `defineManagementModule()` 静态提取
`managementUi(import.meta.url, './ui/index.tsx')`。无 UI declaration 时不会加载 Vite/MF；有 UI 时 server
