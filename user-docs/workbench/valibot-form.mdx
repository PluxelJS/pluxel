---
title: Valibot 配置表单
description: 用一个 Valibot object schema 同时驱动 configs.use、配置元数据和 Web 表单。
---

# Valibot 配置表单

Pluxel 的配置作者入口只有一个 object schema，Plugin 通过 `private readonly config = this.configs.use(Config)` 注册它。

这个 schema 同时承担四件事：类型、默认值、运行时校验，以及给配置 UI/工具链提取的结构。`valibot-form` 只在 schema 上附加显示和布局 metadata，不创建第二套 schema language。
...
```sh
pnpm add valibot-form valibot react react-dom \
  @mantine/core @mantine/hooks @tanstack/react-form \
  @tabler/icons-react @dnd-kit/core @dnd-kit/sortable @dnd-kit/utilities
```
