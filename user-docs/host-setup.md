...
Workbench bundle，可设置 `mode: 'distribution'`。该模式把 bare SSR package import 交给 Node 从已物化 closure 加载，显式
plugin source entry 仍经过 Pluxel transform pipeline。它不会替宿主创建可搬运目录、manifest 或签名；默认开发模式继续使用
