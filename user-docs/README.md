...
1. [`plugin-authoring.md`](plugin-authoring.md)：从标准插件形状开始，掌握依赖、配置、生命周期、
   HTTP、Management Plane 和公开 capability。
2. [`testing.md`](testing.md)：使用 `@pluxel/test/vitest`、core/runtime test host 和 Vitest 验证真实
   插件生命周期、HTTP、失败传播、cleanup 与 disabled Management Plane。
...
| 这是业务 API 还是只服务管理 UI？             | 业务：`ctx.http.plugin`；管理：`ctx.management.mount()`  |
...
- [`host-setup.md`](host-setup.md)：选择 static/dynamic Vite route，配置 Management Plane 和启动策略。
