...
Host 通过 `@pluxel/services/commands` 的 `commands()` 安装空命令目录，Plugin 通过 `ctx.require(Commands)` 取得当前 owner 的服务。官方 Runtime 使用同一安装器，并在准备期间额外发布六个插件管理命令；自定义 Host 不会隐式安装这些管理命令。
