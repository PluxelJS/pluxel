---
title: 插件提交检查
description: 用 13 项检查确认 Plugin 的边界、生命周期、配置和交付契约。
---
...
- [ ] 只服务当前 owner、但需要局部 config/effects 的组成使用 field-only `PluginPart`；Part field initializer 不创建资源，optional activation 的副作用全部留在 `plugins.use()` callback；需要独立治理时使用 Plugin。
