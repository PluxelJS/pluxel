...
| 任务                         | 业务插件依赖 | 本页入口                                  |
| ---------------------------- | ------------ | ----------------------------------------- |
| 调用 Redis 命令或读写 Stream | `Redis`      | [直接使用 Redis](#直接使用-redis)         |
| 原子组合多个命令             | `Redis`      | [Typed Lua scripts](#typed-lua-scripts)   |
| 共享缓存                     | `Cache`      | [作为 Cache backend](#作为-cache-backend) |
| 多进程共享限频额度           | `Rates`      | [作为 Rates backend](#作为-rates-backend) |
