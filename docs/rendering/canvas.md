...
```ts no-twoslash
const raster = this.canvas.createCanvas(1200, 630)

const svg = this.canvas.createSvgCanvas(1200, 630, {
	mode: 'text-to-paths',
})
const svgBytes = svg.getContent()
```

`createSvgCanvas(width, height, options)` 支持三个互斥 mode：
...
const surface = this.canvas.createCanvas(image.width, image.height)
...
默认 `dataOwnership: 'borrowed'`，Canvas 在 native decode 前复制输入。只有 render-local buffer 确定不再使用时才可传：
...
`createImage()` 返回未加载的 native Image placeholder，主要用于 ECharts 等要求同步构造 Image 的 platform adapter。普通图片输入应优先用有预算检查的 `decodeImage()`。
...
```ts no-twoslash
const prepared = this.canvas.prepareText({
...
- `prepareText()`：准备普通文本。
- `prepareTextWithSegments()`：保留手动 Canvas line rendering 所需的 segment 信息。
- `prepareRichInline()`：准备带独立 font、spacing、break 与 extra width 的 inline items。
...
adapter 提供 `createCanvas()`、`createSvgCanvas()`、`createImage()` 与 `decodeImage()`，并重新执行 host budget。snapshot 要求的具体 family 若在线程 native registry 中不存在，会以 `FONT_UNAVAILABLE` 失败。
...
CanvasConfig 不配置并发数、worker queue、render timeout、output bytes、DPR 或主题：
...
- image：`INVALID_IMAGE`、`IMAGE_BYTES_EXCEEDED`。
