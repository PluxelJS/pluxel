...
Plugin 的声明和读取时机见[配置规则](../getting-started/configuration.md#声明规则)。
...
配置分区保留独立的保存基线与撤销范围；保存当前分区不会清空其他分区的草稿。当前保存和全部保存共享请求流程，
请求结束前不能启动另一笔配置保存，但可以继续编辑。网络失败后保留草稿并允许重试；保存已确认成功时，
后续运行状态刷新失败不会将这笔保存报告为失败。
...
`formOpts.defaultValues` 未提供或为 `undefined` 时，使用 schema 默认值。控件、context 和重置共享同一次默认值解析结果，
因此动态默认值工厂生成的值也保持一致。`AutoForm.Actions` 提供的 `setValues` 按 TanStack 字段路径逐项更新，
例如 `setValues({ 'database.host': 'db.example' })`；它保留其他值，不把带点的 key 当作顶层字面量名称。
