...
- 职责：插件 UI/RPC/SSE/扩展点等“交互协议层”。
- 位置：`packages/runtime/src/services/plugin-interaction/*`

其中：

- `ExtService`：`ctx.ext` 聚合入口，只负责把当前 plugin ctx 回灌到 `ui/rpc/sse` 三个子服务
  - `packages/runtime/src/services/plugin-interaction/ExtService.ts`
- `ExtensionService`：runtime 对 UI extension 的稳定入口，同时内建 manifest、builtin registry、compiled module store
  - `packages/runtime/src/services/plugin-interaction/ExtensionService.ts`
- `getModuleSource()` 只会在 `mode: "compile"` 时尝试触发 `compiler.requestCompile(pluginName)`；`registry-only` 只读内存 store 内容
...
- module `/extensions/*` 路由：从 `ExtensionService` 的内存 store 取 code 并返回 JS module
  - `packages/runtime/src/api/http/extensions.ts`

当前判断：

- 这组实现已经足够稳定，不再继续拆出新的 “registry service / bridge service / coordinator” 概念；
- 如果后续要继续优化，优先做类内私有状态整理，而不是重新增加公开层级；
- 只有当 `ExtensionService` 再次明显膨胀，才考虑把内部状态下沉到私有 helper/state 模块。
...
- 职责：把插件 UI entry bundle 成 browser module，并写入 runtime 的 `ExtensionModuleStore`。
- 位置：`packages/hmr/src/dev/extensions/ExtensionCompilerService.ts`
- 依赖：
  - Node FS（磁盘 cache、watcher target 扫描）
  - runtime internal helpers（vendor rewrite / JSX normalize / broken-bundle check）
    - 来自 `@pluxel/runtime/internal`
- 缓存策略：
  - 进程内：如果 `store.getCompiledModule(pluginName)?.sourceHash` 命中则跳过编译
  - 进程重启：优先从磁盘 cache（默认 `.pluxel/extensions/<plugin>/<hash>.mjs`）恢复到内存 store，避免重复编译

当前判断：

- `ExtensionCompilerService` 继续留在 `@pluxel/hmr` 是对的；
- runtime 只保留 `ExtensionModuleStore` bridge，已经是比较合适的最小注入面；
- 暂不为了“更纯粹的命名”再抽象出新的 host/control-plane/compiler 接口层。
