...
先确认项目根目录、相关 package 的 scripts，以及本次要修改的插件或应用入口。已知目标时直接定位，不必先扫描整个项目。
...
1. **定位。** 使用已知文件或 Plugin 查询，沿返回的 declaration、schema、binding 位置阅读源码。修改应用输入时明确选择应用。
2. **确认契约。** 检查当前导出、类型、文档和真实调用方，确定谁拥有状态、失败和清理。
3. **实现与验证。** 运行相关 package 实际声明的检查。脚本存在不代表覆盖本次风险；根据行为选择类型、运行时或生命周期验证。
4. **确认需要的结果。** 编辑后重新查询源码；需要在线效果时检查对应实例的运行结果、应用报告与日志。源码 revision 和隔离测试不能替代在线证据。
