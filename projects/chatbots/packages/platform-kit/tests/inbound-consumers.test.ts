...
import { InboundConsumerRegistry } from '../src/inbound-consumers.ts'

describe('InboundConsumerRegistry', () => {
	it('uses a stable ordered plan for each dispatch', async () => {
		const registry = new InboundConsumerRegistry<string, number>('test consumer')
		const seen: string[] = []
		let disposeSecond = () => {}
	...
	it('is fail-fast and leaves checkpoint policy to the caller', async () => {
		const registry = new InboundConsumerRegistry<object, object>('test consumer')
		const later = vi.fn()
	...
	it('does not start another consumer after cancellation', async () => {
		const registry = new InboundConsumerRegistry<object, object>('test consumer')
