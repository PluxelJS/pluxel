								...
								<Group justify="space-between" wrap="nowrap">
									<Stack gap={0} style={{ minWidth: 0 }}>
										...
										<Text size="xs" c="dimmed" truncate>
											{status?.username ? `@${status.username}` : '尚未鉴权'}
										</Text>
									</Stack>
									<Group gap="xs" wrap="nowrap">
										<Text size="xs" c="dimmed">
											{relativeTime(status?.lastUpdateAt ?? null)}
										</Text>
...
export function TelegramManagerPanel() {
	...
	const run = async (
		...
		try {
			await action()
			setError(null)
			host.notify({ title: 'Telegram', message: success, tone: 'success' })
			return true
		} catch (caught) {
		...
		<Stack gap="lg" p={{ base: 'sm', sm: 'md' }}>
			<Group justify="space-between" align="flex-start" wrap="wrap">
				<Group gap="sm" align="flex-start">
					...
					<Stack gap={2}>
						<Title order={3}>Telegram Bots</Title>
						<Text size="sm" c="dimmed">
							管理多个 Bot 的凭据、Polling 连接和运行健康度
						</Text>
					</Stack>
				</Group>
				<Group gap="xs">
					...
					<Stack gap="lg">
						...
						{selectedSettings && selectedStatus ? (
							<SimpleGrid cols={{ base: 2, sm: 3, lg: 6 }} spacing="sm">
								<Metric
									label="Polling offset"
									value={String((selectedStatus.lastUpdateId ?? -1) + 1)}
									icon={<IconActivity size={16} />}
								/>
								<Metric
									label="连续失败"
									value={String(selectedStatus.consecutiveFailures)}
									tone={selectedStatus.consecutiveFailures ? 'red' : undefined}
								/>
								<Metric label="最近 Polling" value={relativeTime(selectedStatus.lastPollAt)} />
								<Metric
									label="最近更新"
									value={relativeTime(selectedStatus.lastUpdateAt)}
									icon={<IconClock size={16} />}
								/>
								<Metric
									label="当前退避"
									value={
										selectedStatus.currentBackoffMs ? `${selectedStatus.currentBackoffMs} ms` : '-'
									}
									tone={selectedStatus.currentBackoffMs ? 'red' : undefined}
								/>
			...
			<Group justify="space-between" wrap="nowrap" align="flex-start">
				<Group gap="sm" wrap="nowrap" style={{ minWidth: 0 }}>
					...
					<Stack gap={1} style={{ minWidth: 0 }}>
						...
						<Text size="xs" c="dimmed" truncate>
							{status?.username ? `@${status.username}` : (status?.botId ?? '尚未鉴权')}
						</Text>
					</Stack>
				</Group>
				<Badge size="xs" variant="light" color={phase.color}>
...
function AccountHeader({
	...
	return (
		<Group justify="space-between" align="flex-start" wrap="wrap">
			<Stack gap={2}>
				<Title order={4}>{creating ? '新建 Telegram Bot' : (settings?.id ?? '选择一个 Bot')}</Title>
				<Text size="sm" c="dimmed">
					{creating
						? '使用稳定的 Bot ID 区分业务用途。'
						: status?.username
							? `@${status.username} · Telegram ID ${status.botId}`
							: '保存凭据后将读取 Bot 身份。'}
				</Text>
			</Stack>
			{!creating && settings ? (
...
function nextAccountId(settings: readonly TelegramSettingsDoc[]): string {
...
function phaseOrder(status?: TelegramStatusDoc): number {
