...
| `adapter-kit`  | Bot registry、账号存储、确认型 projection、串行/重试/取消原语  | Hub 路由、平台状态机   |
...
| `telegram-hub` | Telegram codec、确认型入站投影与 transport                     | 账号和连接生命周期     |
| `kook`         | KOOK API、Bot registry、原始 event、gateway 心跳重连、账号管理 | ChatMessage 与 Hub     |
| `kook-hub`     | KOOK codec、确认型入站投影与 transport                         | 账号和连接生命周期     |
...
13. adapter 只从 `adapter-kit` 的明确子入口共享 registry、账号存储、串行器、retry gate、纯 `ExponentialBackoff`、可取消 delay 与 superseding abort lease，不通过 Hub 获取这些原语，也不共享连接基类。WebSocket、SSE 和 long polling 各自拥有状态机。
...
15. Telegram Bot API 的方法参数、返回值和对象模型以 `@gramio/types` 为权威；平台包只维护构建期 HTTP method inventory 与 HTTP transport，ChatMessage codec 属于 `telegram-hub`。
