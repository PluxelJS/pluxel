...
import { createKookClient, type KookClientOptions, type KookRawApi } from '../api/client.ts'
...
import { createKookTools } from '../api/tools.ts'
import { dispatchKookEvent } from './events.dispatch.ts'
import { createKookBotEvents } from './events.factory.ts'
import type { KookBotEvents, KookEvent, KookPluginEvents } from './events.types.ts'
...
export type KookBotOptions = Omit<KookClientOptions, 'signal'> & {
	...
	consumeEvent?: (bot: KookBot, event: KookEvent, signal: AbortSignal) => void | Promise<void>
...
export class KookBot extends KookNativeApi {
	...
	constructor(botOptions: KookBotOptions) {
		...
		const extensions: KookBotExtensions = {
			...
			channel: tools.createConversation,
			direct: tools.createDirectConversation,
			createAsset: tools.createAsset,
			start: () => this.connect(),
	...
	private async connect(): Promise<KookBotStatus> {
		...
		try {
			const identity = unwrap(
				await this.#api.$.raw.call('getUserMe', undefined, { signal: lease.signal }),
			)
			lease.throwIfStale()
			...
			this.gateway = new KookGateway(
				{
					getUrl: async (request, signal) => {
						const value = unwrap(
							await this.#api.$.raw.call('getGateway', { compress: 0 }, { signal }),
						)
					...
					onEvent: async (event, signal) => {
						...
						await this.#options.consumeEvent?.(this, event, signal)
					},
					onOnline: (sessionId) => {
