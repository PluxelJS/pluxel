...
type Trim<S extends string> = S extends ` ${infer A}` ? Trim<A>
  : S extends `${infer A} ` ? Trim<A> : S
...
// —— 汇总 —— //
export type ExtractCommandParams<P extends string> =
  ParseRequired<P> & ParseOptional<P> & ParseRest<P>
...
export function parseArgsStringToArgv(input: string): string[] {
  ...
  while (i < input.length) {
    ...
    if (quote) {
      ...
      if (ch === '\\') { if (i < input.length) cur += input[i++] ; continue }
      cur += ch
    }
  }
  if (cur) out.push(cur)
...
export function defineCommand<P extends string, F extends Flags, C = unknown>(
  ...
  const runCore = async (tokens: string[], ctx: C) => {
    ...
    for (let i = 0; i < cp.optional.length; i++) {
      const v = pos[k]
      if (v !== undefined) { params[cp.optional[i]] = v; k++ }
    }
  ...
  return {
    ...
    aliases,
    toUsage,
    runTokens(tokens, ctx) { return runCore(tokens, ctx) },
    run(input, ctx) { return runCore(parseArgsStringToArgv(input), ctx) },
  }
}
...
export function createCommandBus<C = unknown>(opts?: {
  ...
  return {
    ...
    async dispatch(input: string, ctx: C): Promise<string | void> {
      let tokens = parseArgsStringToArgv(input)
      ...
      if (!found) throw new CommandError(`Unknown command: ${tokens[0]}`)
