...
export interface CommandBuilder<P extends string, F extends Flags, C> {
  ...
  action(handler: (argv: TypeFlag<F> & ExtractCommandParams<P>, ctx: C) => Awaitable<string | void>): Command<P, F, C>
}
...
export function createCommandKit<C>(
  bus: { register: (cmd: Command<any, any, C>) => any; list: () => Command<any, any, C>[] },
): CommandKit<C> {
  ...
  const kit: CommandKit<C> = {
    cmd<P extends string, F extends Flags = {}>(
      ...
      const builder: CommandBuilder<P, F, C> = {
        alias(...names) { base.aliases.push(...names); return this },
        usage(text) { usageText = text; return this },
        describe(text) { local.desc = text; return this },
        use(mw) { local.mws.push(mw); return this },
        action(handler) {
    ...
    group(name, def) {
      ...
      try { def(kit) } finally { groupStack.pop() }
    },

    list() { return bus.list() },
