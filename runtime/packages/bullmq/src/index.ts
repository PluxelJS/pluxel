...
export class BullMQPlugin extends BasePlugin {
  ...
  @Config(DashboardConfig) private board!: v.InferOutput<
    typeof DashboardConfig
  >;
  @Config(DefaultsConfig) private defaults!: v.InferOutput<
    typeof DefaultsConfig
  >;
  ...
  get redisClient(): IORedis {
    if (!this.shared)
      throw new Error("[BullMQ] shared connection not established");
    return this.shared;
  }
  ...
  override async init(abort: AbortSignal): Promise<void> {
    if (!this.redis.lazy) {
      await this.raceAbortAndTimeout(
        abort,
        this.ensureSharedReady(),
        this.redis.startupTimeoutMs!
      );
    }
    ...
    this.ctx.honoService.modifyApp((app) => {
      app.route(basePath, adapter.registerPlugin() as any);
    });
  }

  async stop(): Promise<void> {
    await Promise.allSettled(
      [...this.workersAny.values()].map((w) => w.close())
    );
    await Promise.allSettled(
      [...this.eventsMap.values()].map((e) => e.close())
    );
    await Promise.allSettled([...this.queues.values()].map((q) => q.close()));
    if (this.flowProducer) {
      await this.flowProducer.close().catch(() => {});
  ...
  queue(
    name: string,
    opts?: Omit<QueueOptions, "connection" | "prefix">
  ): Queue {
  ...
  worker<T = any, R = any, N extends string = string>(
    ...
    const key = `${queueName}:${opts?.concurrency ?? "default"}:${String(
      processor
    )}`;
    const cached = this.workersAny.get(key) as unknown as
      | Worker<T, R, N>
      | undefined;
  ...
  private getOrCreateShared(): IORedis {
    ...
    c.on("reconnecting", () =>
      this.ctx.logger.info("[BullMQ] shared reconnecting")
    );
    this.shared = c;
    return c;
  }
  ...
  private createBlocking(): IORedis {
    ...
    c.on("reconnecting", () =>
      this.ctx.logger.info("[BullMQ] blocking reconnecting")
    );
  ...
  private async raceAbortAndTimeout<T>(
    ...
    const pAbort = new Promise<never>((_, rej) => {
      const l = () => {
        abort.removeEventListener("abort", l);
        rej(new Error("[BullMQ] init aborted"));
      };
      abort.addEventListener("abort", l);
    });
    const pTimeout = new Promise<never>((_, rej) =>
      setTimeout(
        () => rej(new Error("[BullMQ] init timeout")),
        timeoutMs
      ).unref?.()
    );
