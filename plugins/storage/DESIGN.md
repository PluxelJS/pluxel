...
```text
consumer -> S3 token -> S3Plugin -> bounded named bucket catalog
...
`S3` 是普通 abstract capability token，发布 `bucket(id)` / `bucketIds()`；返回的 `S3Bucket.client` 使用
从 `S3mini` 自动取得、排除下划线 transport internals 的完整作者面，
...
是一个 provider-owned、配置驱动的 bounded domain collection，不产生额外 Plugin identity、config owner 或 dependency edge。

只有实现拥有不同的外部资源生命周期或 s3mini 无法表达的平台认证/transport 时，才新增 `@Plugin(S3, ...)` provider。

## 配置、secret 与生命周期

`S3Config.buckets` 包含 1–64 个唯一 stable ID；每项的 `backend` 是判别式 union：
...
启动阶段并行构造全部 bucket，每个资源成功后立即登记 cleanup；任一项失败会取消 sibling、等待全部 settle 并回滚，不发布部分
catalog。runtime 使用 `Map<bucketId, state>` 做 O(1) lookup；每个 caller Context 的 handle 缓存随 consumer effects 清理。
...
Workbench Content 只是运行中 provider 的可选 operations 投影；credential action 仅在选中的 remote/vault bucket 可用。动态 bucket
只是 rows，不改变 Workbench definition、entry、RPC root 或 socket。它不属于 S3 业务能力，也不是其启动前提。
