...
describe('S3Plugin local backend', () => {
	it('preserves S3 keys and object metadata across provider lifecycles', async () => {
		...
		await withLocalS3(root, async (s3) => {
			expect(await s3.client.bucketExists()).toBe(true)
			expect(await s3.client.createBucket()).toBe(false)
			for (const key of keys) {
				const response = await s3.client.putObject(
					key,
					`body:${key}`,
			...
			const response = await s3.client.getObjectResponse('documents/你好.txt')
		...
		await withLocalS3(root, async (s3) => {
			for (const key of keys) expect(await s3.client.objectExists(key)).toBe(true)
			const listed = await s3.client.listObjects('\u0001')
			expect(listed?.map((object) => object.Key).sort()).toEqual([...keys].sort())
			await expect(s3.client.deleteObject('../escape')).resolves.toBe(true)
			await expect(s3.client.deleteObject('../escape')).resolves.toBe(true)
		})
	})
	...
	it('matches s3mini listing, ranged-read, copy, move, and batch-delete shapes', async () => {
		...
		await withLocalS3(root, async (s3) => {
			for (const key of ['files/a', 'files/deep/b', 'files/deep/c', 'other/z']) {
				await s3.client.putAnyObject(key, key)
			}

			const grouped = await s3.client.listObjects('/', 'files/')
			expect(grouped?.map((object) => object.Key)).toEqual(['files/a', 'files/deep/'])

			const first = await s3.client.listObjectsPaged('\u0001', 'files/', 2)
			...
			const second = await s3.client.listObjectsPaged(
				'\u0001',
				'files/',
				2,
			...
			const range = await s3.client.getObjectRaw('files/deep/b', false, 6, 10)
			...
			const copy = await s3.client.getObjectResponse('copy/a')
			expect(copy?.headers.get('x-amz-meta-copied')).toBe('yes')
			expect(copy?.headers.get('content-type')).toBe('text/custom')

			await s3.client.moveObject('copy/a', 'moved/a')
			expect(await s3.client.objectExists('copy/a')).toBe(false)
			expect(await s3.client.objectExists('moved/a')).toBe(true)
			expect(await s3.client.deleteObjects(['moved/a', 'other/z'])).toEqual([true, true])
		})
	})
	...
	it('does not publish a streamed object whose declared length is wrong', async () => {
		...
		await withLocalS3(root, async (s3) => {
			...
			expect(await s3.client.objectExists('broken.bin')).toBe(false)
		})
	})
	...
	it('supports explicit multipart uploads and reports unsupported S3 semantics honestly', async () => {
		const root = await temporaryRoot()
		await withLocalS3(root, async (s3) => {
			const uploadId = await s3.client.getMultipartUploadId('large.bin', 'application/octet-stream')
			const one = await s3.client.uploadPart('large.bin', uploadId, new Uint8Array([1, 2]), 1)
			const two = await s3.client.uploadPart('large.bin', uploadId, new Uint8Array([3, 4]), 2)
			const uploads = await s3.client.listMultipartUploads()
			...
			const complete = await s3.client.completeMultipartUpload('large.bin', uploadId, [one, two])
			expect(complete).toMatchObject({
				location: 'local-s3:///test-bucket/large.bin',
				bucket: 'test-bucket',
				key: 'large.bin',
			})
			expect(new Uint8Array((await s3.client.getObjectArrayBuffer('large.bin'))!)).toEqual(
				new Uint8Array([1, 2, 3, 4]),
			)
			const abortedId = await s3.client.getMultipartUploadId('aborted.bin')
			await expect(s3.client.abortMultipartUpload('aborted.bin', abortedId)).resolves.toMatchObject(
				{
					status: 'Aborted',
					key: 'aborted.bin',
					uploadId: abortedId,
			...
			await expect(s3.client.getPresignedUrl('GET', 'large.bin')).rejects.toMatchObject({
				name: S3UnsupportedOperationError.name,
				code: 'S3_UNSUPPORTED_OPERATION',
			})
			await expect(s3.client.setBucketVersioning('Enabled')).rejects.toBeInstanceOf(
