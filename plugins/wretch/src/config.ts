...
export const WretchConfig = v.object({
	timeoutMs: v.pipe(
		...
		f.formMeta({ label: '请求超时', description: '每次底层 fetch attempt 的超时（毫秒）' }),
		f.numberMeta({ min: 0, step: 1_000, integer: true }),
	),
	maxConcurrentRequests: v.pipe(
		v.optional(v.pipe(v.number(), v.integer(), v.minValue(1)), 64),
		f.formMeta({ label: '最大并发请求', description: '所有 consumer 共用的 fetch attempt 上限' }),
		f.numberMeta({ min: 1, step: 1, integer: true }),
	),
	maxQueuedRequests: v.pipe(
		v.optional(v.pipe(v.number(), v.integer(), v.minValue(0)), 256),
		f.formMeta({ label: '最大等待请求', description: '并发满时允许进入等待队列的请求数' }),
		f.numberMeta({ min: 0, step: 1, integer: true }),
	),
	allowedOrigins: v.pipe(
		v.optional(v.array(HttpOrigin), []),
		f.formMeta({ label: '允许的 Origins', description: '空列表不限制；非空时使用精确 origin' }),
